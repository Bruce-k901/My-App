-- ============================================================================
-- CONSOLIDATED CRON MIGRATION - Run this in Supabase Dashboard SQL Editor
-- ============================================================================
-- This migration updates the cron job to use Active Tasks (checklist_tasks)
-- as the source instead of task_templates, and runs at 7:15 AM UTC for testing.
-- ============================================================================

-- Drop existing cron jobs (idempotent)
DO $$
BEGIN
...
-- Drop the daily-only function if it exists (we don't need it anymore)
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      BEGIN
        ...
        ELSE
          v_should_generate := (v_day_of_week = 1); -- Default to Monday
        END IF;
        ...
        IF v_existing_task_id IS NULL THEN
          INSERT INTO checklist_tasks (
            ...
            v_task_pattern.template_id, v_task_pattern.company_id, v_task_pattern.site_id,
            v_today, v_task_pattern.due_time, v_task_pattern.daypart,
            v_task_pattern.assigned_to_role, v_task_pattern.assigned_to_user_id,
            'pending', COALESCE(v_task_pattern.priority, 'medium'),
  ...
  FOR v_task_pattern IN
    SELECT DISTINCT ON (ct.template_id, ct.site_id, ct.daypart, ct.due_time)
      ct.template_id, ct.company_id, ct.site_id, ct.daypart, ct.due_time,
      ct.assigned_to_role, ct.assigned_to_user_id, ct.priority, ct.task_data,
      tt.frequency, tt.recurrence_pattern, tt.is_active
    FROM checklist_tasks ct
    INNER JOIN task_templates tt ON ct.template_id = tt.id
  ...
  LOOP
    BEGIN
      ...
      BEGIN
        ...
        ELSE
          v_should_generate := (v_date_of_month = 1); -- Default to 1st
        END IF;
