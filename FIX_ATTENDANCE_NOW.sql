-- ============================================================================
-- CRITICAL FIX: Remove ALL sync triggers that cause attendance_logs errors
-- Copy this entire file and paste into Supabase SQL Editor, then click Run
-- ============================================================================

BEGIN;

-- Drop ALL possible sync triggers on staff_attendance
...
-- Drop ALL possible sync triggers on attendance_logs (including INSTEAD OF triggers)
...
-- Drop the sync functions (they're no longer needed since we use a view)
...
-- Drop any function with similar names (catch-all)
DO $$
DECLARE
  r RECORD;
BEGIN
...
-- Drop any RLS policies on attendance_logs that allow INSERT/UPDATE/DELETE
...
-- Ensure the view exists and is read-only
...
-- Create a function that explicitly prevents inserts (for extra safety)
CREATE OR REPLACE FUNCTION prevent_attendance_logs_insert()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  RAISE EXCEPTION 'Cannot insert into attendance_logs view. Use staff_attendance table instead.';
END;
$$;

-- This should never fire since views can't have BEFORE INSERT triggers, but just in case
-- (PostgreSQL doesn't allow triggers on views for INSERT/UPDATE/DELETE without INSTEAD OF)
