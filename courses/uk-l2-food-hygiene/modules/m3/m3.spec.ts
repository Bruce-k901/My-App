...
describe("Module m3 content", () => {
  it("module manifest validates", () => {
    const manifest = loadJson("module.json");
    const parsed = safeParseOrThrow(moduleManifestSchema, manifest, "module m3 manifest");
    expect(parsed.id).toBe("m3");
    expect(parsed.pages.length).toBeGreaterThan(0);
  });

  it("pages validate and include lottie/hotspot", () => {
    const pages = loadJson("pages.json");
    const parsed = safeParseOrThrow(pagesSchema, pages, "module m3 pages");
    expect(parsed.some((page) => page.type === "lottie")).toBe(true);
    expect(parsed.some((page) => page.type === "hotspot")).toBe(true);
    expect(parsed.find((page) => page.id === "m3_p8")?.type).toBe("quiz_ref");
  });

  it("quiz pool validates", () => {
    const quiz = loadJson("quiz.json");
    const parsed = safeParseOrThrow(quizSchema, quiz, "module m3 quiz");
    expect(parsed.pool_id).toBe("m3");
    expect(parsed.items.length).toBe(6);
  });

  it("outcomes mapping validates", () => {
    const outcomes = loadJson("outcomes.json");
    const parsed = safeParseOrThrow(outcomesMappingSchema, outcomes, "module m3 outcomes");
