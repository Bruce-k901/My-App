...
describe("Module m7 content", () => {
  it("module manifest validates", () => {
    const manifest = loadJson("module.json");
    const parsed = safeParseOrThrow(moduleManifestSchema, manifest, "module m7 manifest");
    expect(parsed.id).toBe("m7");
    expect(parsed.pages.length).toBe(8);
  });

  it("pages validate and include lottie/recap", () => {
    const pages = loadJson("pages.json");
    const parsed = safeParseOrThrow(pagesSchema, pages, "module m7 pages");
    expect(parsed.some((page) => page.type === "lottie")).toBe(true);
    expect(parsed.some((page) => page.type === "recap")).toBe(true);
    expect(parsed.at(-1)?.type).toBe("quiz_ref");
  });

  it("quiz pool validates", () => {
    const quiz = loadJson("quiz.json");
    const parsed = safeParseOrThrow(quizSchema, quiz, "module m7 quiz");
    expect(parsed.pool_id).toBe("m7");
    expect(parsed.items.length).toBe(5);
  });

  it("outcomes mapping validates", () => {
    const outcomes = loadJson("outcomes.json");
    const parsed = safeParseOrThrow(outcomesMappingSchema, outcomes, "module m7 outcomes");
