  ...
  try {
    ...
    async function createTask(params: any) {
      ...
      if (params.template) {
        ...
        const defaultChecklistItems =
          recurrencePattern?.default_checklist_items || [];
        ...
        if (
          Array.isArray(defaultChecklistItems) &&
          defaultChecklistItems.length > 0
        ) {
          if (hasYesNoChecklist) {
            // Yes/No checklist format
            taskData.yesNoChecklistItems = defaultChecklistItems.map((
              item: any,
            ) => ({
            ...
            taskData.checklistItems = defaultChecklistItems
              .map((item: any) =>
      ...
      // ========================================================================
      if (
        params.equipmentConfig && Array.isArray(params.equipmentConfig) &&
        params.equipmentConfig.length > 0
      ) {
        ...
        const selectedAssets = isArrayOfIds
          ? params.equipmentConfig.filter((id: any) =>
            typeof id === "string" && id.length > 0
          )
          : params.equipmentConfig
            .map((item: any) =>
              item.id || item.asset_id || item.value ||
        ...
        if (params.template?.repeatable_field_name) {
          ...
          if (isArrayOfIds) {
            ...
            taskData[repeatableFieldName] = params.equipmentConfig.map((
              id: string,
            ) => ({
              value: id,
              asset_id: id,
        ...
        if (
          !isArrayOfIds && params.equipmentConfig[0] &&
          typeof params.equipmentConfig[0] === "object"
        ) {
          // Check if there are temperature fields
          const firstItem = params.equipmentConfig[0];
          if (
            firstItem.temp !== undefined || firstItem.temperature !== undefined
          ) {
      ...
      if (params.template?.evidence_types?.includes("temperature")) {
        ...
        if (
          ...
          (Array.isArray(taskData.temperatures) &&
            taskData.temperatures.length === 0)
        ) {
          if (
            taskData.selectedAssets && Array.isArray(taskData.selectedAssets) &&
            taskData.selectedAssets.length > 0
          ) {
            // Initialize temperatures array with one entry per selected asset
            taskData.temperatures = taskData.selectedAssets.map((
              assetId: string,
            ) => ({
              assetId: assetId,
              temp: null,
      ...
      try {
        ...
        if (config.daypart_times && typeof config.daypart_times === "object") {
          for (const [daypart, times] of Object.entries(config.daypart_times)) {
            ...
            for (const time of timeArray as any[]) {
    ...
    try {
      ...
      const { data: overdueAssets } = await supabase
        ...
        .or(
          `last_service_date.lt.${sixMonthsAgo.toISOString()},next_service_date.lte.${todayString}`,
        )
        .eq("status", "active");
    ...
    try {
      ...
      if (certTemplate) {
        ...
        const { data: allProfiles } = await supabase
          .from("profiles")
          .select(
            "id, full_name, site_id, company_id, food_safety_expiry_date, h_and_s_expiry_date, fire_marshal_expiry_date, first_aid_expiry_date, cossh_expiry_date, food_safety_level, h_and_s_level",
          );
        ...
        for (const profile of allProfiles || []) {
          ...
          for (const cert of certificates) {
            ...
            const certTypeLabel = cert.type === "food_safety"
              ? "Food Safety"
              : cert.type === "h_and_s"
              ? "Health & Safety"
              : cert.type === "fire_marshal"
              ? "Fire Marshal"
              : cert.type === "first_aid"
              ? "First Aid"
              : "COSHH";
            ...
            const taskName =
              `${certTypeLabel}${levelText} Certificate Expiring: ${
    ...
    try {
      ...
      if (sopTemplate) {
        ...
        const { data: sops } = await supabase
          .from("sop_entries")
          .select(
            "id, title, ref_code, company_id, site_id, sop_data, updated_at, created_at",
          );
        ...
        for (const sop of sops || []) {
          ...
          let reviewDate = null;
          if (
            sop.sop_data && typeof sop.sop_data === "object" &&
            sop.sop_data.review_date
          ) {
          ...
          const taskName = `SOP Review Due: ${sop.title || "SOP"} (${
            sop.ref_code || "N/A"
          })`;
    ...
    try {
      ...
      if (raTemplate) {
        ...
        const { data: riskAssessments } = await supabase
          ...
          .select(
            "id, title, ref_code, company_id, site_id, next_review_date, status",
          )
        ...
        for (const ra of riskAssessments || []) {
          ...
          const taskName = `Risk Assessment Review Due: ${ra.title || "RA"} (${
            ra.ref_code || "N/A"
          })`;
      ...
      if (messagingTasks && messagingTasks.length > 0) {
        ...
        for (const msgTask of messagingTasks) {
          ...
          const { error } = await supabase.from("checklist_tasks").insert({
            ...
            due_time: null,
            daypart: "anytime",
            status: msgTask.status === "in_progress"
              ? "in_progress"
              : "pending",
    ...
    try {
      ...
      if (docTemplate) {
        ...
        for (const doc of expiringDocs || []) {
          ...
          const taskName =
            `Document Review Due: ${doc.name}${versionText} - ${doc.category}`;
    ...
    return new Response(
      JSON.stringify({
        ...
        ...log,
        total_tasks_created: log.daily_tasks_created +
