...
Deno.serve(async () => {
  ...
  await supabase.from("notifications").update({ status: "archived" }).lt("created_at", cutoff).eq("status", "active");
  ...
  const { data: settings } = await supabase
    ...
    .select("user_id, receive_email_digests, include_incidents, include_tasks, notify_temperature_warnings");
  ...
  for (const i of incidents || []) incMap.set((i as any).site_id, Number((i as any).count));
  const taskMap = new Map<string, number>();
  for (const t of tasks || []) taskMap.set((t as any).site_id, Number((t as any).count));
  const tempMap = new Map<string, number>();
  for (const x of tempFailed || []) tempMap.set((x as any).site_id, Number((x as any).count));
  ...
  for (const r of recipients || []) {
    ...
    const lines: string[] = [];
    lines.push(`Daily Digest for ${role === "admin" ? "All Sites" : "Your Site"}:\n`);
    ...
    if (SENDGRID_KEY && s.receive_email_digests && email) {
      try {
        const siteName = siteId
          ? ((await supabase.from("sites").select("name").eq("id", siteId).limit(1)).data?.[0]?.name || siteId)
          : "All Sites";
        const html = `
          <div style="font-family: Inter,ui-sans-serif,system-ui,Arial; color:#0f172a">
            <div style="padding:16px; background:#0f1220; border:1px solid #1f2937; border-radius:12px;">
              ...
              <ul style="margin:12px 0; padding-left:20px; color:#e5e7eb">
                ${s.include_incidents ? `<li>Open Incidents: <strong>${openIncidents}</strong></li>` : ""}
