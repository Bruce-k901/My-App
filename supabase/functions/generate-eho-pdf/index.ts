...
serve(async (req) => {
  ...
  try {
    ...
    const { site_id, start_date, end_date, categories, include_missed }: EHOReportRequest = await req.json()
    ...
    return new Response(html, {
      headers: {
        ...
        'Content-Disposition': `inline; filename="eho-report-${start_date}-to-${end_date}.html"`
      }
    })
...
function generatePDFHTML(data: {
  ...
  const { 
    site, company, startDate, endDate, summary, completions,
    trainingRecords, temperatureRecords, incidentReports,
    cleaningRecords, pestControlRecords, openingClosingChecklists,
    supplierDeliveryRecords, maintenanceLogs, staffHealthDeclarations,
    allergenInformation, generatedAt 
  } = data
...
<body>
  ...
  <div class="header">
    ...
    <div class="header-info">
      ...
      <strong>Address:</strong> ${site?.address || 'N/A'} ${site?.postcode || ''}<br>
  ...
  <div class="summary-section">
    ...
    <div class="summary-grid">
      ...
      <div class="summary-card">
        ...
        <div class="value" style="color: ${completionRate >= 90 ? '#4caf50' : completionRate >= 70 ? '#ff9800' : '#f44336'};">${completionRate}%</div>
      </div>
    </div>
    ...
    <div class="category-breakdown">
      ...
      ${summary.map(cat => `
        <div class="category-item">
          <span><strong>${cat.category.replace('_', ' ').toUpperCase()}</strong></span>
          <span>${cat.completed_tasks} / ${cat.total_tasks} (${cat.completion_rate.toFixed(1)}%)</span>
        </div>
      `).join('')}
    </div>
  </div>
  ...
  <div class="completions-section">
    ...
    ${completions.map((comp, idx) => `
      <div class="completion-item">
        ...
        <div class="completion-meta">
          <strong>Completed by:</strong> ${comp.completed_by_name || 'Unknown'} (${comp.completed_by_role || 'N/A'})<br>
        ...
        <div class="completion-data">
          <table>
            ${comp.completion_data ? Object.entries(comp.completion_data).filter(([key]) => 
      ...
      <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
        ...
        <tbody>
          ${trainingRecords.map(tr => `
            <tr>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${tr.staff_name || 'Unknown'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${tr.training_type || 'N/A'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${new Date(tr.completed_at).toLocaleDateString()}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${tr.expiry_date ? new Date(tr.expiry_date).toLocaleDateString() : 'N/A'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${tr.certificate_number || 'N/A'}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
      ...
      <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
        ...
        <tbody>
          ${temperatureRecords.map(tr => `
            <tr>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${new Date(tr.recorded_at).toLocaleString()}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${tr.asset_name || 'Unknown'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${tr.reading} ${tr.unit || '°C'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">
                <span style="color: ${tr.status === 'breach' ? '#f44336' : tr.status === 'warning' ? '#ff9800' : '#4caf50'};">
                  ${tr.status?.toUpperCase() || 'OK'}
                </span>
              </td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${tr.recorded_by_name || 'Unknown'}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  <div class="incidents-section" style="margin-top: 40px; page-break-before: always;">
    ...
    ${incidentReports.length > 0 ? incidentReports.map(inc => `
      <div class="completion-item">
        <div class="completion-header">
          <h3>${inc.incident_type || 'Incident'}</h3>
          <div class="completion-meta">${new Date(inc.occurred_at).toLocaleString()}</div>
        </div>
        <div class="completion-meta">
          ...
          ${inc.riddor_category ? ` | <strong>RIDDOR:</strong> ${inc.riddor_category}` : ''}
        </div>
        <div class="completion-data">
          <p><strong>Description:</strong> ${inc.description || 'No description provided'}</p>
          ${inc.follow_up_actions ? `<p><strong>Follow-up Actions:</strong> ${inc.follow_up_actions}</p>` : ''}
        </div>
      </div>
    `).join('') : '<p style="color: #666; font-style: italic;">No incident reports found for this period.</p>'}
  </div>

  <div class="cleaning-section" style="margin-top: 40px; page-break-before: always;">
    <h2>Cleaning Schedules & Completion Logs</h2>
    ${cleaningRecords.length > 0 ? cleaningRecords.map(clean => `
      <div class="completion-item">
        <div class="completion-header">
          <h3>${clean.template_name || 'Cleaning Task'}</h3>
          <div class="completion-meta">${new Date(clean.completed_at).toLocaleString()}</div>
        </div>
        <div class="completion-meta">
          <strong>Completed by:</strong> ${clean.completed_by_name || 'Unknown'} | 
          <strong>Due:</strong> ${clean.due_date} ${clean.daypart || ''}
        </div>
        <div class="completion-data">
          <table>
            ${clean.completion_data ? Object.entries(clean.completion_data).filter(([key]) => 
    ...
    `).join('') : '<p style="color: #666; font-style: italic;">No cleaning records found for this period.</p>'}
  </div>

  <div class="pest-control-section" style="margin-top: 40px; page-break-before: always;">
    <h2>Pest Control Records</h2>
    ${pestControlRecords.length > 0 ? pestControlRecords.map(pest => `
      <div class="completion-item">
        <div class="completion-header">
          <h3>Pest Control Inspection</h3>
          <div class="completion-meta">${new Date(pest.completed_at).toLocaleString()}</div>
        </div>
        <div class="completion-meta">
        ...
        <div class="completion-data">
          <p><strong>Assessment:</strong> 
            <span style="color: ${pest.assessment_result === 'pass' ? '#4caf50' : '#f44336'};">
              ${pest.assessment_result?.toUpperCase() || 'N/A'}
            </span>
          </p>
          ${pest.findings ? `<p><strong>Findings:</strong> ${pest.findings}</p>` : ''}
          ${pest.actions_taken ? `<p><strong>Actions Taken:</strong> ${pest.actions_taken}</p>` : ''}
        </div>
      </div>
    `).join('') : '<p style="color: #666; font-style: italic;">No pest control records found for this period.</p>'}
  </div>

  <div class="checklists-section" style="margin-top: 40px; page-break-before: always;">
    <h2>Opening & Closing Checklists</h2>
    ${openingClosingChecklists.length > 0 ? openingClosingChecklists.map(checklist => `
      <div class="completion-item">
        <div class="completion-header">
          <h3>${checklist.checklist_type || 'Checklist'}</h3>
          <div class="completion-meta">${new Date(checklist.completed_at).toLocaleString()}</div>
        </div>
        <div class="completion-meta">
          <strong>Completed by:</strong> ${checklist.completed_by_name || 'Unknown'} | 
          <strong>Daypart:</strong> ${checklist.daypart || 'N/A'}
        </div>
        <div class="completion-data">
          <table>
            ${checklist.completion_data ? Object.entries(checklist.completion_data).filter(([key]) => 
              !['photos', 'evidence_attachments'].includes(key)
            ).map(([key, value]) => `
              <tr>
                <td>${key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}:</td>
    ...
    `).join('') : '<p style="color: #666; font-style: italic;">No opening/closing checklists found for this period.</p>'}
  </div>
      ...
      <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
        ...
        <tbody>
          ${supplierDeliveryRecords.map(del => `
            <tr>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${del.delivery_date ? new Date(del.delivery_date).toLocaleDateString() : 'N/A'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${del.supplier_name || 'Unknown'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${del.items_received || 'N/A'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${del.received_by_name || 'Unknown'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${del.temperature_check || 'N/A'}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  <div class="maintenance-section" style="margin-top: 40px; page-break-before: always;">
    ...
    ${maintenanceLogs.length > 0 ? maintenanceLogs.map(maint => `
      <div class="completion-item">
        <div class="completion-header">
          <h3>${maint.maintenance_type || 'Maintenance'} - ${maint.asset_name || 'Unknown Asset'}</h3>
          <div class="completion-meta">${maint.completed_at ? new Date(maint.completed_at).toLocaleString() : 'N/A'}</div>
        </div>
        <div class="completion-meta">
          <strong>Completed by:</strong> ${maint.completed_by_name || 'Unknown'}
          ${maint.next_due_date ? ` | <strong>Next Due:</strong> ${new Date(maint.next_due_date).toLocaleDateString()}` : ''}
        </div>
        <div class="completion-data">
          <p><strong>Description:</strong> ${maint.description || 'No description provided'}</p>
        </div>
      </div>
    `).join('') : '<p style="color: #666; font-style: italic;">No maintenance logs found for this period. (Feature pending implementation)</p>'}
  </div>
      ...
      <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
        ...
        <tbody>
          ${staffHealthDeclarations.map(decl => `
            <tr>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${decl.declaration_date ? new Date(decl.declaration_date).toLocaleDateString() : 'N/A'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${decl.staff_name || 'Unknown'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${decl.health_status || 'N/A'}</td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">
                <span style="color: ${decl.fit_for_work ? '#4caf50' : '#f44336'};">
                  ${decl.fit_for_work ? 'YES' : 'NO'}
                </span>
              </td>
              <td style="padding: 8px; border-bottom: 1px solid #eee;">${decl.symptoms || 'None reported'}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  <div class="allergen-section" style="margin-top: 40px; page-break-before: always;">
    ...
    ${allergenInformation.length > 0 ? allergenInformation.map(allergen => `
      <div class="completion-item">
        <div class="completion-header">
          <h3>${allergen.allergen_name || 'Allergen'}</h3>
          ${allergen.last_updated ? `<div class="completion-meta">Last Updated: ${new Date(allergen.last_updated).toLocaleDateString()}</div>` : ''}
        </div>
        <div class="completion-data">
          ${allergen.present_in_items && allergen.present_in_items.length > 0 ? `
            <p><strong>Present in:</strong> ${allergen.present_in_items.join(', ')}</p>
          ` : ''}
          ${allergen.procedures ? `<p><strong>Procedures:</strong> ${allergen.procedures}</p>` : ''}
        </div>
      </div>
    `).join('') : '<p style="color: #666; font-style: italic;">No allergen information available. (Feature pending implementation)</p>'}
  </div>
