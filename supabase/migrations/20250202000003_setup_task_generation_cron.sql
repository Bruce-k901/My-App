-- Migration: Setup Automated Task Generation Cron Job
-- Description: Creates a pg_cron job to automatically generate tasks every day at 3am UTC
-- This ensures tasks are populated automatically without manual intervention
...
-- Create a function that generates tasks directly (without HTTP call)
...
-- Schedule the cron job to run every day at 3:00 AM UTC
...
BEGIN
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'generate-daily-tasks-cron') THEN
    RAISE NOTICE '✅ Cron job "generate-daily-tasks-cron" scheduled successfully for 3:00 AM UTC daily';
  ELSE
