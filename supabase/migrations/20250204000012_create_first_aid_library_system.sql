...
-- Only create tables if companies table exists
DO $$
DECLARE
...
BEGIN
  ...
  notes TEXT,
    ...
    -- RLS Policies for first_aid_supplies_library (only if profiles table exists)
    ...
    -- RLS Policies for first_aid_requirements (only if profiles table exists)
  ...
  ELSE
    RAISE NOTICE '⚠️ companies table does not exist yet - skipping first aid library system creation';
  END IF;
END $$;
