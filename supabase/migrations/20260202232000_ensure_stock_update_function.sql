-- ============================================================================
-- Migration: Ensure stock update function exists
-- Description: Re-creates the update_stock_on_delivery_confirm function
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Calculate new average cost (use weighted average by default)
    IF v_current_avg_cost IS NOT NULL AND v_current_qty > 0 THEN
      -- Weighted average: (old_qty * old_avg + new_qty * new_cost) / (old_qty + new_qty)
    ...
    ELSIF v_unit_cost > 0 THEN
      -- First purchase - use the unit cost as avg
    ...
    IF v_line.variant_id IS NOT NULL AND v_unit_cost > 0 THEN
      ...
      IF v_line.variant_current_price IS NULL OR v_line.variant_current_price != v_unit_cost THEN
        -- Create price history record (only if price_history table has the columns)
        BEGIN
        ...
        EXCEPTION WHEN others THEN
          -- Log but don't fail if price_history insert fails
