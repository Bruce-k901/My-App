-- ============================================================================
-- Migration: Fix stock calculation to use ingredient pack_size
-- Description: When updating stock from deliveries, convert quantity received
--              (in packs) to base units using ingredients_library.pack_size
-- ============================================================================
...
DECLARE
  ...
  v_library_type TEXT;
  v_old_unit_cost DECIMAL(12,4);
  v_old_pack_cost DECIMAL(12,4);
  v_old_current_stock DECIMAL(12,4);
  v_new_total_stock DECIMAL(12,4);
...
BEGIN
  ...
  LOOP
    ...
    -- Get the library item and pack_size for proper conversion
    SELECT si.library_item_id, si.library_type
    INTO v_library_item_id, v_library_type
    FROM stockly.stock_items si
    WHERE si.id = v_stock_item_id;

    -- Get pack_size from ingredients_library for conversion
    -- This converts packs to base units (e.g., 1 pack of 1000g = 1000 base units)
    v_pack_size := 1; -- Default to 1 if not found
    IF v_library_item_id IS NOT NULL AND v_library_type = 'ingredients_library' THEN
      SELECT COALESCE(NULLIF(pack_size, '')::NUMERIC, 1)
      INTO v_pack_size
      FROM public.ingredients_library
      WHERE id = v_library_item_id;

      -- Ensure pack_size is at least 1
      IF v_pack_size IS NULL OR v_pack_size <= 0 THEN
        v_pack_size := 1;
      END IF;
    END IF;

    -- Calculate base quantity: packs received × pack_size = base units
    -- e.g., 1 pack × 1000g = 1000g
    v_base_qty := v_quantity * v_pack_size;
    ...
    -- Calculate unit cost per base unit (e.g., £12.80 per 1000g = £0.0128 per gram)
    -- For stock movements, we record per-base-unit cost
    DECLARE
      v_unit_cost_per_base DECIMAL(10,6);
    BEGIN
      v_unit_cost_per_base := v_unit_cost / NULLIF(v_pack_size, 0);

      -- Calculate new average cost (weighted average) per base unit
      IF v_current_avg_cost IS NOT NULL AND v_current_qty > 0 THEN
        v_new_avg_cost := ((v_current_qty * v_current_avg_cost) + (v_base_qty * v_unit_cost_per_base))
                          / (v_current_qty + v_base_qty);
      ELSIF v_unit_cost_per_base > 0 THEN
        v_new_avg_cost := v_unit_cost_per_base;
      ELSE
    ...
    BEGIN
      INSERT INTO stockly.stock_movements (
        ...
        v_stock_item_id,
        'purchase',
        v_base_qty,  -- Now in base units (e.g., grams)
        v_delivery.site_id,
        v_unit_cost / NULLIF(v_pack_size, 0),  -- Cost per base unit
        v_unit_cost * v_quantity,  -- Total cost for all packs
        'delivery_line',
        v_line.id,
        format('From delivery: %s (qty: %s packs × %s = %s base units)',
               COALESCE(v_delivery.invoice_number, v_delivery.id::text),
               v_quantity, v_pack_size, v_base_qty),
        p_user_id,
        NOW()
      );
    ...
    -- Update or insert stock level in stockly schema
    BEGIN
    ...
    IF v_line.variant_id IS NOT NULL AND v_unit_cost > 0 THEN
      IF v_line.variant_current_price IS NULL OR v_line.variant_current_price != v_unit_cost THEN
        -- Record price history for product variant
        BEGIN
    ...
    -- Update ingredients_library if stock_item is linked to an ingredient
    IF v_library_item_id IS NOT NULL AND v_library_type = 'ingredients_library' THEN
      BEGIN
        -- Get old values for price history and current stock
        SELECT unit_cost, pack_cost, COALESCE(current_stock, 0)
        INTO v_old_unit_cost, v_old_pack_cost, v_old_current_stock
        FROM public.ingredients_library
        WHERE id = v_library_item_id;

        -- Calculate new total stock in base units
        v_new_total_stock := v_old_current_stock + v_base_qty;

        -- Calculate change percent for price
        IF v_old_pack_cost IS NOT NULL AND v_old_pack_cost > 0 THEN
          v_change_percent := ((v_unit_cost - v_old_pack_cost) / v_old_pack_cost * 100);
        ELSE
          v_change_percent := NULL;
        END IF;

        -- Log price change if price changed
        IF v_unit_cost > 0 AND (v_old_pack_cost IS NULL OR v_old_pack_cost != v_unit_cost) THEN
          INSERT INTO stockly.ingredient_price_history (
            ...
            v_library_item_id,
            v_old_unit_cost,
            v_unit_cost / NULLIF(v_pack_size, 0),
            v_old_pack_cost,
            v_unit_cost,
            v_change_percent,
            'invoice',
            'delivery:' || p_delivery_id::text,
            NOW(),
            p_user_id
          );
        END IF;

        -- Update the ingredient with new price AND new stock level (in base units)
        UPDATE public.ingredients_library
        SET pack_cost = v_unit_cost,
            unit_cost = v_unit_cost / NULLIF(v_pack_size, 0),
        ...
        END IF;
      EXCEPTION WHEN OTHERS THEN
        v_errors := array_append(v_errors, 'Ingredient update failed: ' || SQLERRM);
      END;
    END IF;
...
-- Also update the recalculate function to use pack_size
...
BEGIN
  ...
  WITH movement_totals AS (
    ...
    -- Movements should already be in base units if created correctly
    SELECT
      sm.stock_item_id,
  ...
  SET
    quantity = GREATEST(0, mt.calculated_qty),
  ...
  WHERE sl.stock_item_id = mt.stock_item_id
    ...
    AND ABS(sl.quantity - mt.calculated_qty) > 0.001
  RETURNING
    sl.stock_item_id,
    sl.site_id,
