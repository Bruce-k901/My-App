-- Fix taken_days and pending_days to be calculated from leave_requests table
-- instead of relying on static values in leave_balances (triggers may not have fired)
...
BEGIN
  ...
  -- Compute holiday year boundaries for the requested year
  v_hy_start := make_date(p_year, v_month, v_day);
