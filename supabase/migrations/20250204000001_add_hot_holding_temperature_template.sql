...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'task_templates')
    ...
    SELECT t.id, 'equipment_name', 'repeatable_record', 'Equipment Name', true, 1, 'Select the hot holding equipment being checked'
    FROM task_templates t
    WHERE t.slug = 'hot_holding_temperature_verification'
      AND NOT EXISTS (
        SELECT 1 FROM public.template_fields tf 
        WHERE tf.template_id = t.id AND tf.field_name = 'equipment_name'
      );
    ...
    SELECT t.id, 'temperature', 'number', 'Temperature (°C)', true, 63, 100, 65, 63, 2, 'Hot holding must be above 63°C. Recheck immediately if below 63°C'
    FROM task_templates t
    WHERE t.slug = 'hot_holding_temperature_verification'
      AND NOT EXISTS (
        SELECT 1 FROM public.template_fields tf 
    ...
    -- Notes field
    INSERT INTO public.template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
    SELECT t.id, 'notes', 'text', 'Notes', false, 3, 'Record any observations or actions taken'
    FROM task_templates t
    WHERE t.slug = 'hot_holding_temperature_verification'
      AND NOT EXISTS (
        SELECT 1 FROM public.template_fields tf 
