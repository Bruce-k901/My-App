-- ============================================================================
-- Migration: Clean up stock movement notes format
-- Description: Simplify the notes to be user-friendly without technical details
-- ============================================================================

-- Update the delivery confirm function with cleaner notes
...
BEGIN
  ...
  LOOP
    ...
    -- SIMPLIFIED NOTES: Just show the delivery reference, no conversion math
    BEGIN
      INSERT INTO stockly.stock_movements (
        ...
        v_stock_item_id,
        'purchase',
        v_base_qty,  -- In base units (e.g., grams)
        v_delivery.site_id,
        v_unit_cost / NULLIF(v_pack_size, 0),  -- Cost per base unit
        v_unit_cost * v_quantity,  -- Total cost for all packs
        'delivery_line',
        v_line.id,
        'Delivery ' || COALESCE(v_delivery.invoice_number, '#' || LEFT(v_delivery.id::text, 8)),
        p_user_id,
        NOW()
      );
