-- ============================================================================
-- Migration: Update ingredients_library.current_stock from delivery
-- Description: Sync current_stock field when stock levels are updated
-- ============================================================================
  ...
  LOOP
    ...
    IF v_unit_cost > 0 THEN
      BEGIN
        ...
        -- If linked to ingredients_library, update pack_cost, unit_cost, AND current_stock
        IF v_library_item_id IS NOT NULL AND v_library_type = 'ingredients_library' THEN
          ...
          -- Calculate new total stock (old + received quantity)
          ...
          IF v_old_pack_cost IS NULL OR v_old_pack_cost != v_unit_cost THEN
            INSERT INTO stockly.ingredient_price_history (
              ...
              v_library_item_id,
              v_old_unit_cost,
              v_unit_cost / NULLIF(NULLIF((SELECT pack_size FROM public.ingredients_library WHERE id = v_library_item_id), '')::numeric, 0),
              v_old_pack_cost,
              v_unit_cost,
              v_change_percent,
              'invoice',
          ...
          -- Update the ingredient with new price AND new stock level
