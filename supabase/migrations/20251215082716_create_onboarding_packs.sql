...
CREATE TABLE IF NOT EXISTS public.company_onboarding_packs (
  ...
  -- Base packs represent core policies that should apply to many roles (typically boh_foh='BOTH').
...
-- ---------------------------------------------------------------------------
-- Template documents (personalised contracts/offer letters/etc.)
-- These attach template text + merge field metadata to a global_documents row.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.global_document_templates (
  ...
  global_document_id UUID NOT NULL REFERENCES public.global_documents(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  -- Store as TEXT (not HTML) for safety. You can later render to PDF / DocuSign.
  template_body TEXT NOT NULL,
  -- JSON array of fields, e.g. [{\"key\":\"start_date\",\"label\":\"Start date\",\"type\":\"date\"}, ...]
...
-- Per-employee rendered instances (snapshots) for template documents
...
-- Assignments: company members can read assignments for their company;
-- employees can read their own assignments; admins/managers can create.
...
-- Acknowledgements: employee can insert for themselves; company can read
...
-- Templates: company members can read; admins/managers can write
...
-- Instances: employee can read their own; company can read; admins/managers can create/update
