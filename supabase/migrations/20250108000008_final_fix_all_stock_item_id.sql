...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'stockly schema does not exist - skipping final_fix_all_stock_item_id migration';
    RETURN;
  END IF;
  
  RAISE NOTICE 'stockly schema found - proceeding with final_fix_all_stock_item_id migration';
END $$;
...
BEGIN
  ...
  -- Drop ALL possible trigger names (only if view exists)
  IF EXISTS (
  ...
  -- Recreate triggers (only if view exists)
  IF EXISTS (
    SELECT 1 FROM information_schema.views 
    WHERE table_schema = 'public' 
    AND table_name = 'recipe_ingredients'
