-- ============================================================================
-- Migration: 20251215134000_fix_seed_company_wfm_starter_kit_category_check.sql
-- Description:
--   Hotfix: global_documents has a category CHECK constraint with an unknown
--   allowed set in some environments.
--
--   This recreates seed_company_wfm_starter_kit so inserts will:
--   - Try the preferred category
--   - On check_violation (23514), fall back through a list of known categories
--     until one succeeds.
-- ============================================================================
...
BEGIN
  ...
  -- Recreate the seed function with category fallback.
  ...
  BEGIN
    ...
    FOR pr IN (
      ...
      -- Pack docs (helper function handles idempotency)
  ...
  RAISE NOTICE 'Updated seed_company_wfm_starter_kit to fallback categories on check violations';
END $do$;
