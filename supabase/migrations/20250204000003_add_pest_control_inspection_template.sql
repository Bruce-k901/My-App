...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'task_templates')
    ...
    SELECT t.id, 'overall_assessment', 'pass_fail', 'Overall Assessment', true, 1, 'FAIL if any pest activity is detected. This will trigger a pest control contractor callout.'
    FROM task_templates t
    WHERE t.slug = 'weekly_pest_control_inspection'
      AND NOT EXISTS (
        SELECT 1 FROM public.template_fields tf 
        WHERE tf.template_id = t.id AND tf.field_name = 'overall_assessment'
      );

    -- Notes field (for additional observations)
    INSERT INTO public.template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
    SELECT t.id, 'notes', 'text', 'Additional Notes', false, 2, 'Record any additional observations or actions taken'
    FROM task_templates t
    WHERE t.slug = 'weekly_pest_control_inspection'
      AND NOT EXISTS (
        SELECT 1 FROM public.template_fields tf 
        WHERE tf.template_id = t.id AND tf.field_name = 'notes'
      );
  END IF;
END $$;
...
-- Verification (only if task_templates exists)
DO $$
DECLARE
  template_id UUID;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'task_templates') THEN
    ...
    IF template_id IS NOT NULL THEN
      ...
      IF NOT (SELECT triggers_contractor_on_failure FROM task_templates WHERE id = template_id) THEN
        RAISE WARNING '⚠️  Template created but triggers_contractor_on_failure is not set to TRUE';
      END IF;
      
      IF NOT (SELECT contractor_type = 'pest_control' FROM task_templates WHERE id = template_id) THEN
        RAISE WARNING '⚠️  Template created but contractor_type is not set to pest_control';
      END IF;
    ...
    ELSE
      RAISE NOTICE '⚠️  Template not found - may need to run previous migrations first';
    END IF;
  ELSE
