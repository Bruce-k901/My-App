...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'staff_attendance') THEN
    ...
    RAISE NOTICE 'Updated attendance RPC functions to use staff_attendance';

  ELSE
    RAISE NOTICE '⚠️ staff_attendance table does not exist yet - skipping function updates';
  END IF;
END $$;
