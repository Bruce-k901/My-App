-- Migration: Fix task generation concatenation error
-- Description: Fixes "operator does not exist: integer || integer" error
-- Issue: due_time concatenation needs explicit text casting
-- Date: 2025-02-06

-- Fix the generate_daily_tasks_direct function
    ...
    BEGIN
      ...
      BEGIN
        ...
        LOOP
          ...
          BEGIN
            -- FIX: Explicitly cast due_time to text, handle NULL properly
            SELECT array_agg(DISTINCT COALESCE(daypart::text, '') || '|' || COALESCE(due_time::text, '')) 
              ...
              BEGIN
                ...
                IF v_times_for_daypart IS NULL OR array_length(v_times_for_daypart, 1) IS NULL THEN
                  IF v_template.time_of_day IS NOT NULL THEN
                    v_times_for_daypart := ARRAY[v_template.time_of_day::text];
                  ELSE
                ...
                LOOP
                  DECLARE
                    v_combination TEXT := COALESCE(v_daypart::text, '') || '|' || COALESCE(v_time_str, '');
                  BEGIN
      ...
      BEGIN
        ...
        LOOP
          ...
          BEGIN
            ...
            -- FIX: Explicitly cast due_time to text
            SELECT array_agg(DISTINCT COALESCE(daypart::text, '') || '|' || COALESCE(due_time::text, '')) 
            INTO v_existing_combinations
            FROM checklist_tasks
            WHERE template_id = v_template.id
...
FIXED: Concatenation error by explicitly casting daypart and due_time to text.';
