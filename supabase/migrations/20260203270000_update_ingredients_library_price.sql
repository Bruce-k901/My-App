-- ============================================================================
-- Migration: Update stock function to also update ingredients_library prices
-- Description: When delivery is confirmed, update the ingredients_library pack_cost
-- ============================================================================
...
BEGIN
  -- Get delivery info using public view (which maps to stockly schema)
  ...
  -- Process each delivery line using public view
  FOR v_line IN
    SELECT
      dl.id,
      dl.delivery_id,
    ...
    LEFT JOIN public.product_variants pv ON dl.product_variant_id = pv.id
  ...
  LOOP
    ...
    BEGIN
      ...
      DO UPDATE SET
        ...
        total_value = (public.stock_levels.quantity + EXCLUDED.quantity) * COALESCE(v_new_avg_cost, public.stock_levels.avg_cost, 0),
    ...
    -- Update product variant price if changed
    ...
    -- NEW: Update ingredients_library price if stock_item is linked to an ingredient
