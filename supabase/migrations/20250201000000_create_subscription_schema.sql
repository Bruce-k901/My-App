...
-- Note: Tables that reference companies will be skipped if companies table doesn't exist yet

-- Subscription plans table (can be created independently - doesn't need companies)
...
-- Company subscriptions table (only if companies table exists)
DO $$
BEGIN
...
-- Invoices table (for manual invoicing) - only if companies table exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'companies') THEN
...
-- Data export requests table (for GDPR/compliance) - only if companies table exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'companies') THEN
...
-- Company subscriptions: Users can view their company's subscription (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'company_subscriptions') THEN
...
-- Invoices: Users can view their company's invoices (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'invoices') THEN
...
-- Data export requests: Users can view their company's export requests (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'data_export_requests') THEN
...
-- Functions and triggers (only create if company_subscriptions table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'company_subscriptions') THEN
...
-- Updated_at triggers (only if tables exist)
DO $$
BEGIN
