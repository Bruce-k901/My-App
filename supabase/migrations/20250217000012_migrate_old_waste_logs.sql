...
BEGIN
  ...
  IF NOT table_exists THEN
    RAISE NOTICE '⚠️ waste_logs table does not exist yet - skipping migration. Will be created by migration 20250217000006.';
    RETURN;
  END IF;
  ...
  -- Get row count (only if table exists)
  SELECT COUNT(*) INTO row_count FROM public.waste_logs;
  ...
  IF has_old_schema AND NOT has_new_schema THEN
    ...
    -- Add foreign key for recorded_by if profiles table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'profiles') THEN
      IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_schema = 'public' 
        AND table_name = 'waste_logs' 
        AND constraint_name = 'waste_logs_recorded_by_fkey'
      ) THEN
        ALTER TABLE public.waste_logs 
    ...
    IF NOT EXISTS (
      ...
      WHERE constraint_schema = 'public' 
      AND table_name = 'waste_logs' 
      AND constraint_name = 'waste_logs_waste_reason_check'
    ) THEN
      ALTER TABLE public.waste_logs 
    ...
    -- Ensure company_id is populated (if sites table exists)
  ...
  ELSIF has_new_schema THEN
    ...
    IF NOT EXISTS (
      ...
      ALTER TABLE public.waste_logs ADD COLUMN company_id UUID;
    END IF;
