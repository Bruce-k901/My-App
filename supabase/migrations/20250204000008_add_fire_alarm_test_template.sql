...
SELECT id, 'test_date', 'date', 'Test Date', true, 1, 'Date when the fire alarm test was performed'
FROM task_templates WHERE slug = 'fire_alarm_test_weekly';

-- Fire Alarm Call Point field (select dropdown - not asset-based)
-- Note: Call points are stored as template_repeatable_labels, not as assets
INSERT INTO public.template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
SELECT id, 'fire_alarm_call_point', 'select', 'Fire Alarm Call Point', true, 2, 'Select the call point being tested this week. Rotate through all call points.'
FROM task_templates WHERE slug = 'fire_alarm_test_weekly';

-- Alarm Activated Pass/Fail field (CRITICAL - failure triggers callout)
INSERT INTO public.template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
SELECT id, 'alarm_activated', 'pass_fail', 'Alarm Activated Successfully', true, 3, 'PASS if alarm activated when call point was pressed. FAIL if alarm did not activate - this will trigger a fire engineer callout.'
FROM task_templates WHERE slug = 'fire_alarm_test_weekly';

-- All Staff Heard Pass/Fail field
INSERT INTO public.template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
SELECT id, 'all_staff_heard', 'pass_fail', 'All Staff Heard the Alarm', true, 4, 'PASS if all staff confirmed they heard the alarm. FAIL if alarm could not be heard in all areas.'
FROM task_templates WHERE slug = 'fire_alarm_test_weekly';

-- Emergency Lighting Test Pass/Fail field (monthly)
INSERT INTO public.template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
SELECT id, 'emergency_lighting_test', 'pass_fail', 'Emergency Lighting Test (Monthly)', false, 5, 'Test emergency lighting monthly. PASS if all emergency lights activate and are bright enough. FAIL if any lights are dim, flickering, or dead.'
FROM task_templates WHERE slug = 'fire_alarm_test_weekly';

-- Issues/Notes field
INSERT INTO public.template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
SELECT id, 'issues', 'text', 'Issues or Observations', false, 6, 'Record any issues, observations, or additional notes about the test'
FROM task_templates WHERE slug = 'fire_alarm_test_weekly';

-- Manager Initials field
INSERT INTO public.template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
SELECT id, 'manager_initials', 'text', 'Tested By (Initials)', true, 7, 'Initials of the person who performed the test'
FROM task_templates WHERE slug = 'fire_alarm_test_weekly';

-- Add default fire alarm call point labels (users can add more via template editor)
-- These are common call point locations - users should customize these for their venue
...
INSERT INTO public.template_repeatable_labels (template_id, label, label_value, is_default, display_order)
SELECT id, 'Call Point 2 - Kitchen', 'Call Point 2 - Kitchen', true, 2
FROM task_templates WHERE slug = 'fire_alarm_test_weekly';

INSERT INTO public.template_repeatable_labels (template_id, label, label_value, is_default, display_order)
SELECT id, 'Call Point 3 - Bar Area', 'Call Point 3 - Bar Area', true, 3
FROM task_templates WHERE slug = 'fire_alarm_test_weekly';

INSERT INTO public.template_repeatable_labels (template_id, label, label_value, is_default, display_order)
SELECT id, 'Call Point 4 - Back Office', 'Call Point 4 - Back Office', true, 4
FROM task_templates WHERE slug = 'fire_alarm_test_weekly';

INSERT INTO public.template_repeatable_labels (template_id, label, label_value, is_default, display_order)
SELECT id, 'Call Point 5 - Storage Area', 'Call Point 5 - Storage Area', true, 5
FROM task_templates WHERE slug = 'fire_alarm_test_weekly';
