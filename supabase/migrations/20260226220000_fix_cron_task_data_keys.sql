-- ============================================================================
-- Migration: Fix cron task_data keys to match client expectations
-- Problem: Cron Part 1b stores checklist items as 'default_checklist_items'
--          but the client (useTaskState) expects 'checklistItems'.
--          Also, referenceDocuments (template_documents) are never copied.
-- Fix: Add 'checklistItems' and 'referenceDocuments' keys to task_data
--      so cron-generated tasks match manually-created ones.
-- Note: Client-side fallback already handles old tasks — this is belt-and-suspenders.
-- ============================================================================
...
BEGIN
  ...
  -- ===== PART 1b: GENERATE TASKS FROM SITE_CHECKLISTS =====
  -- Scan site_checklists (active=true) and create checklist_tasks for today.
  -- Now includes checklistItems + referenceDocuments from template recurrence_pattern
  -- so tasks render correctly in today's tasks without extra client-side fallback.
  BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'site_checklists') THEN
      DECLARE
        ...
        v_time_val jsonb;
        v_time_text text;
        v_task_data_obj jsonb;
        v_checklist_items jsonb;
        v_reference_docs jsonb;
      BEGIN
        FOR v_sc IN
          SELECT
            sc.id as sc_id,
        ...
        LOOP
          BEGIN
            ...
            IF v_sc_should_create THEN
              -- Pre-build shared task_data components from template
              v_checklist_items := COALESCE(v_sc.recurrence_pattern->'default_checklist_items', '[]'::jsonb);
              v_reference_docs := COALESCE(v_sc.recurrence_pattern->'template_documents', '[]'::jsonb);

              -- Build task_data with both legacy and new key names
              v_task_data_obj := jsonb_build_object(
                'source', 'cron',
                'from_site_checklist', v_sc.sc_id,
                'equipment_config', COALESCE(to_jsonb(v_sc.equipment_config), '[]'::jsonb)
              );

              -- Add checklist items under the key the client expects
              IF v_sc.evidence_types IS NOT NULL AND 'yes_no_checklist' = ANY(v_sc.evidence_types) THEN
                -- Yes/No checklist — store as yesNoChecklistItems
                v_task_data_obj := v_task_data_obj || jsonb_build_object('yesNoChecklistItems', v_checklist_items);
              ELSE
                -- Regular checklist — store as checklistItems
                v_task_data_obj := v_task_data_obj || jsonb_build_object('checklistItems', v_checklist_items);
              END IF;

              -- Add reference documents if template has any
              IF v_reference_docs != '[]'::jsonb THEN
                v_task_data_obj := v_task_data_obj || jsonb_build_object('referenceDocuments', v_reference_docs);
              END IF;
...
'Generates daily task instances. Part 1b now includes checklistItems/yesNoChecklistItems
and referenceDocuments in task_data so tasks render correctly without client fallbacks.';
