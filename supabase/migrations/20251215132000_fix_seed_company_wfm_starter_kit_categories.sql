-- ============================================================================
-- Migration: 20251215132000_fix_seed_company_wfm_starter_kit_categories.sql
-- Description:
--   Hotfix: global_documents has a CHECK constraint on category.
--   Recreate seed_company_wfm_starter_kit to use only allowed categories.
-- ============================================================================
...
BEGIN
  ...
  -- Recreate the seed function with corrected categories.
  ...
  BEGIN
    ...
    FOR r IN (
      SELECT * FROM (
        VALUES
          -- Categories must match global_documents_category_check.
  ...
  RAISE NOTICE 'Updated seed_company_wfm_starter_kit categories to satisfy global_documents_category_check';
END $do$;
