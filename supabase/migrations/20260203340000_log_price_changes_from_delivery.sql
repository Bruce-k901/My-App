-- ============================================================================
-- Migration: Log price changes when updating from delivery confirmation
-- Description: The trigger might not fire from SECURITY DEFINER function,
--              so we log price changes directly in the update function
-- ============================================================================
  ...
  LOOP
    ...
    IF v_unit_cost > 0 THEN
      BEGIN
        ...
        -- If linked to ingredients_library, update the pack_cost and log price change
        IF v_library_item_id IS NOT NULL AND v_library_type = 'ingredients_library' THEN
          -- Get old values for price history
          SELECT unit_cost, pack_cost
          INTO v_old_unit_cost, v_old_pack_cost
          ...
          IF v_old_pack_cost IS NULL OR v_old_pack_cost != v_unit_cost THEN
            -- Log price change BEFORE updating (so we capture old values)
