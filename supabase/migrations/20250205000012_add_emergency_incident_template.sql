...
-- Create emergency incident reporting template (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'task_templates') THEN
...
-- Add template fields for incident reporting (only if both tables exist)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'task_templates')
    ...
    SELECT t.id, 'incident_datetime', 'date', 'Incident Date & Time', TRUE, 1, 
      'Date and time when the incident occurred. Use the exact time if known.'
    FROM task_templates t
    WHERE t.slug = 'emergency_incident_reporting'
      AND NOT EXISTS (SELECT 1 FROM template_fields tf WHERE tf.template_id = t.id AND tf.field_name = 'incident_datetime');

    -- Incident Type
    INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, options)
    SELECT t.id, 'incident_type', 'select', 'Incident Type', TRUE, 2,
    ...
    FROM task_templates t
    WHERE t.slug = 'emergency_incident_reporting'
      AND NOT EXISTS (SELECT 1 FROM template_fields tf WHERE tf.template_id = t.id AND tf.field_name = 'incident_type');

    -- Severity
    INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, options)
    SELECT t.id, 'severity', 'select', 'Incident Severity', TRUE, 3,
    ...
    FROM task_templates t
    WHERE t.slug = 'emergency_incident_reporting'
      AND NOT EXISTS (SELECT 1 FROM template_fields tf WHERE tf.template_id = t.id AND tf.field_name = 'severity');

    -- Location
    INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, placeholder)
    SELECT t.id, 'location', 'text', 'Location of Incident', TRUE, 4,
      'Exact location where the incident occurred (e.g., "Kitchen - Prep Area", "Main Dining Room - Table 12").',
      'e.g., Kitchen - Prep Area'
    FROM task_templates t
    WHERE t.slug = 'emergency_incident_reporting'
      AND NOT EXISTS (SELECT 1 FROM template_fields tf WHERE tf.template_id = t.id AND tf.field_name = 'location');

    -- Description
    INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, placeholder)
    SELECT t.id, 'incident_description', 'text', 'Detailed Description', TRUE, 5,
      'Provide a detailed description of what happened. Include: what was happening, what went wrong, immediate consequences.',
      'Describe the incident in detail...'
    FROM task_templates t
    WHERE t.slug = 'emergency_incident_reporting'
      AND NOT EXISTS (SELECT 1 FROM template_fields tf WHERE tf.template_id = t.id AND tf.field_name = 'incident_description');

    -- Emergency Services
    INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
    SELECT t.id, 'emergency_services_called', 'pass_fail', 'Emergency Services Called', TRUE, 10,
      'PASS if emergency services (999) were called. FAIL if not called (but may be needed).'
    FROM task_templates t
    WHERE t.slug = 'emergency_incident_reporting'
      AND NOT EXISTS (SELECT 1 FROM template_fields tf WHERE tf.template_id = t.id AND tf.field_name = 'emergency_services_called');

    -- First Aid Provided
    INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
    SELECT t.id, 'first_aid_provided', 'pass_fail', 'First Aid Provided', TRUE, 11,
      'PASS if first aid was provided. FAIL if first aid was not provided (but may have been needed).'
    FROM task_templates t
    WHERE t.slug = 'emergency_incident_reporting'
      AND NOT EXISTS (SELECT 1 FROM template_fields tf WHERE tf.template_id = t.id AND tf.field_name = 'first_aid_provided');

    -- Scene Preserved
    INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
    SELECT t.id, 'scene_preserved', 'pass_fail', 'Scene Preserved', TRUE, 12,
      'PASS if the scene was preserved for investigation. FAIL if scene was disturbed before investigation.'
    FROM task_templates t
    WHERE t.slug = 'emergency_incident_reporting'
      AND NOT EXISTS (SELECT 1 FROM template_fields tf WHERE tf.template_id = t.id AND tf.field_name = 'scene_preserved');

    -- Immediate Actions
    INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, placeholder)
    SELECT t.id, 'immediate_actions', 'text', 'Immediate Actions Taken', TRUE, 13,
    ...
    FROM task_templates t
    WHERE t.slug = 'emergency_incident_reporting'
      AND NOT EXISTS (SELECT 1 FROM template_fields tf WHERE tf.template_id = t.id AND tf.field_name = 'immediate_actions');

    -- Reported By
    INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
    SELECT t.id, 'reported_by', 'text', 'Reported By (Name)', TRUE, 20,
      'Name of the person completing this incident report.'
    FROM task_templates t
    WHERE t.slug = 'emergency_incident_reporting'
      AND NOT EXISTS (SELECT 1 FROM template_fields tf WHERE tf.template_id = t.id AND tf.field_name = 'reported_by');
  END IF;
END $$;
