...
-- Create smart training compliance task template
...
SELECT id, 'matrix_link', 'text', 'Open Live Training Matrix', TRUE, 1,
  'Click the button below to open the live Training Matrix in a new tab. Paste the URL you opened for audit trail.',
  '/dashboard/training'
FROM task_templates WHERE slug = 'training_compliance_management';

INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, placeholder)
SELECT id, 'matrix_summary', 'text', 'Training Matrix Summary', TRUE, 2,
...
INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, placeholder)
SELECT id, 'expiring_certificates', 'text', 'Certificates Expiring Within 2 Months', TRUE, 10,
...
INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, placeholder)
SELECT id, 'missing_mandatory_training', 'text', 'Missing Mandatory Training', TRUE, 11,
...
SELECT id, 'specialist_coverage_ok', 'pass_fail', 'Specialist Coverage Adequate', TRUE, 20,
  'PASS if First Aiders and Fire Marshals meet coverage requirements; FAIL if additional training or coverage is needed.'
FROM task_templates WHERE slug = 'training_compliance_management';

INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, placeholder)
SELECT id, 'specialist_coverage_notes', 'text', 'Specialist Coverage Notes', TRUE, 21,
...
INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
SELECT id, 'training_booking_triggers', 'repeatable_record', 'Training Booking Triggers', FALSE, 30,
  'Optional: list each person flagged for refresher/new training and the action taken (auto or manual entry).'
FROM task_templates WHERE slug = 'training_compliance_management';

INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, placeholder)
SELECT id, 'training_bookings', 'text', 'Training Courses Booked', TRUE, 31,
...
SELECT id, 'training_urgency', 'select', 'Training Urgency Level', TRUE, 32,
...
INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, placeholder)
SELECT id, 'follow_up_tasks_created', 'text', 'Follow-up Tasks Created', TRUE, 40,
...
INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
SELECT id, 'generate_follow_ups', 'pass_fail', 'Generate Follow-up Tasks', TRUE, 41,
...
INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text)
SELECT id, 'full_compliance_achieved', 'pass_fail', 'Full Training Compliance Achieved', TRUE, 50,
...
INSERT INTO template_fields (template_id, field_name, field_type, label, required, field_order, help_text, placeholder)
SELECT id, 'compliance_evidence', 'text', 'Compliance Evidence', TRUE, 51,
