-- Storage policies for task_photos bucket

-- Allow authenticated users to SELECT objects in their company path
CREATE POLICY task_photos_select_company
  ON storage.objects
  FOR SELECT
  USING (
...
CREATE POLICY task_photos_insert_company
  ON storage.objects
  FOR INSERT
  WITH CHECK (
    bucket_id = 'task_photos'
...
CREATE POLICY task_photos_update_company
  ON storage.objects
  FOR UPDATE
  USING (
    bucket_id = 'task_photos'
...
CREATE POLICY task_photos_delete_company
  ON storage.objects
  FOR DELETE
  USING (
    bucket_id = 'task_photos'
