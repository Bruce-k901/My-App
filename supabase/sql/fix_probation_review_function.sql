-- ============================================================================
-- Quick Fix: Update schedule_missing_probation_reviews function
-- Run this to fix the app_role enum issue
-- ============================================================================

-- Drop and recreate the function with the correct enum handling
...
BEGIN
  ...
  LOOP
    ...
    IF v_template_id IS NOT NULL AND v_manager_id IS NOT NULL THEN
      -- Check if schedule already exists (double-check)
      ...
      IF v_existing_schedule IS NULL THEN
        ...
        scheduled := true;
      ELSE
        -- Already exists, mark as scheduled
        scheduled := true;
      END IF;
    ELSE
      scheduled := false;
      ...
      IF v_template_id IS NULL THEN
        RAISE NOTICE 'Failed to schedule probation review for employee %: No template found', v_employee.id;
      END IF;
      IF v_manager_id IS NULL THEN
        RAISE NOTICE 'Failed to schedule probation review for employee %: No manager found', v_employee.id;
      END IF;
    END IF;
...
-- Also update the trigger function to be safe
