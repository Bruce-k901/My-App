-- ============================================================================
-- Add a specific user to messaging by email
...
BEGIN
  ...
  IF user_company_id IS NULL THEN
    RAISE EXCEPTION 'User % has no company_id. Cannot add to messaging channel.', user_email;
  END IF;
  
  RAISE NOTICE 'Found user: % (%), Company: %', user_email, user_profile_id, user_company_id;
  ...
  RAISE NOTICE '✅ Added user % to channel %', user_email, default_channel_id;
END $$;

-- Verification
SELECT 
  'Fix complete' as status,
  'User should now have messaging access' as message;
