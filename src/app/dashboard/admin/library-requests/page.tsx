    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-semibold text-theme-primary">Library Requests</h1>
              <p className="text-sm text-theme-tertiary">Review and manage library requests from users</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {pendingCount > 0 && (
            <span className="px-3 py-1 bg-yellow-50 dark:bg-yellow-500/20 text-yellow-700 dark:text-yellow-400 rounded-full text-sm">
      ...
      <div className="flex items-center gap-4">
        ...
        <select
          ...
          className="bg-neutral-800 border border-neutral-600 rounded-lg pl-4 pr-10 py-2 text-theme-primary"
        >
      ...
      {requests.length === 0 ? (
        <div className="bg-neutral-800/50 rounded-xl p-12 text-center border border-theme">
          ...
          <p className="text-theme-tertiary">No library requests found</p>
        </div>
      ) : (
        <div className="space-y-4">
          {requests.map((request) => {
            ...
            return (
              <div
                key={request.id}
                className="bg-neutral-800/50 rounded-xl border border-theme p-6 hover:border-neutral-600 transition cursor-pointer"
              ...
              >
                ...
                {request.fields && Array.isArray(request.fields) && request.fields.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-theme">
                    <p className="text-xs text-theme-tertiary mb-2">Fields ({request.fields.length}):</p>
