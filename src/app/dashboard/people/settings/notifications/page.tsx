    ...
    return (
      <div className="space-y-6">
        <div className="text-center py-12">
          <p className="text-theme-primary/60">Loading notification settings...</p>
        </div>
      </div>
    );
  }
            ...
            <button
              ...
              className={`
                ...
                ${isActive
                  ...
                  : "border-transparent text-theme-primary/60 hover:text-theme-primary dark:hover:text-theme-secondary"
                }
              `}
            >
              {category.label}
              {count > 0 && (
                <span className={`px-2 py-0.5 rounded-full text-xs ${
                  isActive ? 'bg-blue-50 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400' : 'bg-white/10 text-theme-primary/60'
                }`}>
                  {count}
                </span>
              )}
      ...
      <div className="space-y-4">
        {displayTypes.length === 0 ? (
          <div className="bg-theme-surface border border-theme rounded-xl p-8 text-center">
            <p className="text-theme-primary/60">No notifications in this category</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-semibold text-theme-primary">{type.name}</h3>
                      <Switch
                    ...
                    {type.description && (
                      <p className="text-sm text-theme-primary/60">{type.description}</p>
                    )}
                  </div>
                </div>

                {setting.enabled && (
                  <div className="space-y-4 mt-4 pt-4 border-t border-theme">
                    {/* Delivery Channels */}
                    <div>
                      <label className="block text-sm font-medium text-gray-900 dark:text-theme-primary/80 mb-3">
                        Delivery Channels
                      </label>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="flex items-center justify-between p-3 rounded-lg bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.05]">
                          <div className="flex items-center gap-2">
                            <MessageSquare className="w-4 h-4 text-theme-primary/60" />
                            <span className="text-sm text-theme-primary">In-App</span>
                          </div>
                          <Switch
                        ...
                        <div className="flex items-center justify-between p-3 rounded-lg bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.05]">
                          <div className="flex items-center gap-2">
                            <Mail className="w-4 h-4 text-theme-primary/60" />
                            <span className="text-sm text-theme-primary">Email</span>
                          </div>
                          <Switch
                        ...
                        <div className="flex items-center justify-between p-3 rounded-lg bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.05]">
                          <div className="flex items-center gap-2">
                            <Smartphone className="w-4 h-4 text-theme-primary/60" />
                            <span className="text-sm text-theme-primary">Push</span>
                          </div>
                          <Switch
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {Object.entries(setting.timing_config || {}).map(([key, value]) => {
                            ...
                            return (
                              <div key={key}>
                                <label className="block text-xs text-theme-primary/60 mb-1">
                                  {label}
                                </label>
                                <Input
                                  type="number"
