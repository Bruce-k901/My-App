  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-theme-primary">My Onboarding Documents</h1>
          <p className="text-sm text-theme-primary/60">Review and acknowledge your onboarding documents</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-theme-surface border border-theme rounded-xl p-8 text-center">
          ...
          <div className="text-theme-primary font-semibold text-lg">No onboarding packs assigned yet</div>
        ...
        <div className="space-y-6">
          {assignments.map((a) => {
            ...
            return (
              <div key={a.id} className="bg-theme-surface border border-theme rounded-xl p-6">
                {/* Pack Header */}
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex items-start gap-3">
                    <Package className="w-6 h-6 text-module-fg flex-shrink-0 mt-1" />
                    <div>
                      <h2 className="text-theme-primary font-semibold text-lg">{pack?.name || 'Onboarding Pack'}</h2>
                      <div className="text-xs text-theme-primary/50 mt-1">
                        Assigned {new Date(a.sent_at).toLocaleDateString()}
                      </div>
                      {a.message && (
                        <div className="text-sm text-theme-primary/70 mt-2 p-3 bg-gray-50 dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.05] rounded-lg">
                          {a.message}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-module-fg">
                      {totalDocs > 0 ? Math.round((acknowledgedDocs / totalDocs) * 100) : 0}%
                    </div>
                    <div className="text-xs text-theme-primary/50">
                ...
                <div className="mb-6">
                  <div className="h-2 bg-gray-100 dark:bg-white/[0.05] rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-module-fg to-module-fg/70 transition-all duration-500"
                ...
                {/* Documents */}
                <div>
                  <div className="text-sm text-theme-primary/70 mb-3">Documents to Review ({totalDocs})</div>
                  {docs.length === 0 ? (
                    <div className="text-sm text-theme-primary/60 text-center py-4">
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3 flex-1">
                              <FileText
                                className={`w-5 h-5 flex-shrink-0 ${isAck ? 'text-green-600 dark:text-green-400' : 'text-theme-primary/40'}`}
                              />
                              <div className="flex-1 min-w-0">
                                <div className="text-theme-primary font-medium">{gd?.name || 'Document'}</div>
                                <div className="text-xs text-theme-primary/50 mt-1">
                            ...
                            <div className="flex items-center gap-3">
                              <button
                                ...
                                className="text-sm text-module-fg hover:underline flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                              ...
                              {isAck ? (
                                ...
                                <button
                                  ...
                                  className="px-3 py-1.5 bg-theme-muted hover:bg-gray-200 dark:hover:bg-white/15 border border-theme text-theme-primary rounded-lg text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                                >
