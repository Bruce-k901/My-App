  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Tronc Configuration</h1>
          <p className="text-neutral-400">Configure tip distribution points and values</p>
        </div>
      </div>

      {/* Create New Config */}
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6 space-y-4">
        <h2 className="text-lg font-semibold text-white">Create Tronc Configuration</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label htmlFor="period_start" className="text-white">Period Start Date</Label>
            <Input
              id="period_start"
              type="date"
          ...
          <div className="space-y-2">
            <Label htmlFor="period_end" className="text-white">Period End Date</Label>
            <Input
              id="period_end"
              type="date"
          ...
          <div className="space-y-2">
            <Label htmlFor="point_value" className="text-white">Point Value (£ per point)</Label>
            <Input
              id="point_value"
              type="number"
              step="0.01"
              ...
              className="bg-white/[0.05] border-white/[0.12] text-white"
            />
            <p className="text-xs text-neutral-400">How much each point is worth in pounds</p>
          </div>
          
          <div className="space-y-2">
            <Label htmlFor="tronc_pool" className="text-white">Total Tronc Pool (£)</Label>
            <Input
              id="tronc_pool"
              type="number"
              step="0.01"
              ...
              className="bg-white/[0.05] border-white/[0.12] text-white"
            />
            <p className="text-xs text-neutral-400">Total tip pool for this period</p>
          </div>
        </div>
      ...
      {configs.length > 0 && (
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-white/[0.06]">
            {configs.map((config) => (
              <div key={config.id} className="p-4 hover:bg-white/[0.02]">
