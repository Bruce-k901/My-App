  ...
  try {
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-theme-primary">{template.name}</h1>
              <p className="text-sm text-theme-tertiary mt-1">{template.description || 'No description'}</p>
            </div>
          </div>
    ...
    return (
      <div className="bg-theme-surface border border-theme rounded-lg p-12 text-center">
        <p className="text-theme-primary font-medium">Error loading template</p>
        <p className="text-theme-tertiary text-sm mt-1">
          {error instanceof Error ? error.message : 'Please try refreshing the page'}
        </p>
        <p className="text-theme-tertiary text-xs mt-2">Check the server console for details</p>
      </div>
    );
  }
}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-theme-surface border border-theme rounded-lg p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <h3 className="text-sm font-medium text-theme-tertiary mb-2">Template Type</h3>
          ...
          <div>
            <h3 className="text-sm font-medium text-theme-tertiary mb-2">Duration</h3>
            <p className="text-theme-primary">{template.recommended_duration_minutes} minutes</p>
          </div>
          {template.recommended_frequency_days && (
            <div>
              <h3 className="text-sm font-medium text-theme-tertiary mb-2">Frequency</h3>
              <p className="text-theme-primary">Every {template.recommended_frequency_days} days</p>
            </div>
          )}
          <div>
            <h3 className="text-sm font-medium text-theme-tertiary mb-2">Total Sections</h3>
            <p className="text-theme-primary">{template.sections?.length || 0} sections</p>
          </div>
        </div>

        {template.instructions && (
          <div>
            <h3 className="text-sm font-medium text-theme-tertiary mb-2">Instructions</h3>
            <p className="text-theme-primary whitespace-pre-wrap">{template.instructions}</p>
          </div>
        )}

        {template.rationale && (
          <div>
            <h3 className="text-sm font-medium text-theme-tertiary mb-2">Rationale</h3>
            <p className="text-theme-primary whitespace-pre-wrap">{template.rationale}</p>
          </div>
        )}

        {template.expected_outcomes && (
          <div>
            <h3 className="text-sm font-medium text-theme-tertiary mb-2">Expected Outcomes</h3>
            <p className="text-theme-primary whitespace-pre-wrap">{template.expected_outcomes}</p>
          </div>
        )}
      </div>
  ...
  return (
    <div className="bg-theme-surface border border-theme rounded-lg p-6 space-y-4">
      ...
      {sections.map((section: any, index: number) => (
        <div key={section.id} className="bg-gray-50 dark:bg-white/[0.02] border border-theme rounded-lg p-4">
          <h4 className="text-theme-primary font-medium mb-2">
            {index + 1}. {section.title}
          </h4>
          {section.description && (
            <p className="text-theme-tertiary text-sm mb-3">{section.description}</p>
          )}
          {section.questions && section.questions.length > 0 && (
            <div className="space-y-2">
              <p className="text-xs text-theme-tertiary mb-2">Questions ({section.questions.length}):</p>
              {section.questions.map((question: any, qIndex: number) => (
                <div key={question.id} className="pl-4 border-l-2 border-theme">
