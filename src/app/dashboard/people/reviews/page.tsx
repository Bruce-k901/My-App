  ...
  try {
    ...
    return (
      <div className="bg-white dark:bg-white/[0.03] border border-gray-200 dark:border-white/[0.06] rounded-lg p-12 text-center">
        <p className="text-gray-900 dark:text-white font-medium">Error loading reviews</p>
        <p className="text-neutral-400 text-sm mt-1">
          {error instanceof Error ? error.message : 'An unexpected error occurred'}
        </p>
        <p className="text-gray-500 dark:text-white/50 text-xs mt-2">Check the server console for details</p>
        <pre className="text-xs text-gray-500 dark:text-white/50 mt-4 text-left bg-black/20 p-4 rounded overflow-auto">
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="h-10 w-32 bg-gray-100 dark:bg-white/[0.05] rounded animate-pulse" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="h-24 bg-gray-100 dark:bg-white/[0.05] rounded-lg animate-pulse" />
        ))}
      </div>
      <div className="h-10 w-64 bg-gray-100 dark:bg-white/[0.05] rounded animate-pulse" />
      <div className="space-y-4">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="h-20 bg-gray-100 dark:bg-white/[0.05] rounded-lg animate-pulse" />
        ))}
      </div>
    </div>
  );
