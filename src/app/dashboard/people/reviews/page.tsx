  ...
  try {
    ...
    return (
      <div className="bg-theme-surface border border-theme rounded-lg p-12 text-center">
        <p className="text-theme-primary font-medium">Error loading reviews</p>
        ...
        <pre className="text-xs text-theme-tertiary mt-4 text-left bg-black/20 p-4 rounded overflow-auto">
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="h-8 w-48 bg-theme-button rounded animate-pulse" />
        <div className="h-10 w-32 bg-theme-button rounded animate-pulse" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="h-24 bg-theme-button rounded-lg animate-pulse" />
        ))}
      </div>
      <div className="h-10 w-64 bg-theme-button rounded animate-pulse" />
      <div className="space-y-4">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="h-20 bg-theme-button rounded-lg animate-pulse" />
        ))}
      </div>
    </div>
  );
