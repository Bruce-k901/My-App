  ...
  const fetchEmployees = async () => {
    ...
    try {
      ...
      if (!companyId) {
        ...
        console.warn('[warn] Profile has no company_id - cannot fetch employees');
      ...
      console.log('Calling get_company_profiles with company_id:', companyId);
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        console.log('[ok] Sample employee from RPC:', data[0]);
        ...
        console.log('[warn] Data is object, not array:', data);
      ...
      if (!data) {
        ...
        console.warn('[warn] RPC returned null data');
      ...
      if (data.length === 0) {
        ...
        console.log('[info] No employees found in database for company:', companyId);
      ...
      if (siteIds.length > 0) {
        ...
        if (sitesError) {
          ...
          console.log('[ok] Fetched sites:', sitesData?.length || 0, sitesData);

          if (sitesData && sitesData.length > 0) {

            sitesMap = new Map(sitesData.map((s: any) => [String(s.id), s.name]));

            console.log('[map] Sites map created with', sitesMap.size, 'entries:', Array.from(sitesMap.entries()));

          } else {

            console.warn('[warn] Sites query returned empty array');

            console.log('Site IDs we searched for:', siteIds);

            // Try fetching all sites to see if RLS is blocking

            console.log('Testing RLS - fetching all sites for company:', companyId);
            ...
            if (allSitesError) {
              ...
              console.log('[ok] Found sites via company_id filter:', allSites);
              ...
              if (matchedSites.length > 0) {
                ...
                console.log('[ok] Rebuilt sites map from company filter:', sitesMap.size);
        ...
        console.log('[warn] No site IDs found in employee data');
      ...
      const formatted = filteredData.map((emp: any, index: number) => {
        ...
        if (homeSiteId && !siteName) {
          ...
          console.warn('[warn] Employee has home_site but site_name not found:', {
