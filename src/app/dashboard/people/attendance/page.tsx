...
export default function AttendancePage() {
  ...
  const getStatusBadge = (status: string, isLate: boolean, isOnBreak: boolean) => {
    if (isOnBreak) return <span className="px-2 py-0.5 bg-amber-500/20 text-amber-400 rounded text-xs border border-amber-500/30">On Break</span>;
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {loading ? (
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-lg overflow-hidden overflow-x-auto">
              <table className="w-full min-w-[700px]">
                <thead className="bg-white/[0.05]">
                  <tr>
                    <th className="px-4 py-3 text-left text-sm font-medium text-neutral-400">Employee</th>
                    <th className="px-3 py-3 text-center text-sm font-medium text-neutral-400">Mon</th>
                    <th className="px-3 py-3 text-center text-sm font-medium text-neutral-400">Tue</th>
                    <th className="px-3 py-3 text-center text-sm font-medium text-neutral-400">Wed</th>
                    <th className="px-3 py-3 text-center text-sm font-medium text-neutral-400">Thu</th>
                    <th className="px-3 py-3 text-center text-sm font-medium text-neutral-400">Fri</th>
                    <th className="px-3 py-3 text-center text-sm font-medium text-neutral-400">Sat</th>
                    <th className="px-3 py-3 text-center text-sm font-medium text-neutral-400">Sun</th>
                    <th className="px-4 py-3 text-right text-sm font-medium text-neutral-400">Total</th>
                    <th className="px-4 py-3 text-right text-sm font-medium text-neutral-400">OT</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/[0.06]">
                  {weeklyData.map((row) => (
                    <tr key={row.profile_id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-3 py-3 text-center text-neutral-300">{row.mon_hours ? row.mon_hours.toFixed(1) : '-'}</td>
                      <td className="px-3 py-3 text-center text-neutral-300">{row.tue_hours ? row.tue_hours.toFixed(1) : '-'}</td>
                      <td className="px-3 py-3 text-center text-neutral-300">{row.wed_hours ? row.wed_hours.toFixed(1) : '-'}</td>
                      <td className="px-3 py-3 text-center text-neutral-300">{row.thu_hours ? row.thu_hours.toFixed(1) : '-'}</td>
                      <td className="px-3 py-3 text-center text-neutral-300">{row.fri_hours ? row.fri_hours.toFixed(1) : '-'}</td>
                      <td className="px-3 py-3 text-center text-neutral-300">{row.sat_hours ? row.sat_hours.toFixed(1) : '-'}</td>
                      <td className="px-3 py-3 text-center text-neutral-300">{row.sun_hours ? row.sun_hours.toFixed(1) : '-'}</td>
                      <td className="px-4 py-3 text-right text-white font-medium">{row.total_hours.toFixed(1)}</td>
                      <td className={`px-4 py-3 text-right font-medium ${row.overtime_hours > 0 ? 'text-amber-400' : 'text-neutral-500'}`}>
