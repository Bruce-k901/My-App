    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-neutral-400">Track compliance and certification expiry</p>
        </div>
      ...
      {overview && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-green-400">{overview.fully_compliant}</p>
          </div>
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-lg p-4">
            <div className="flex items-center gap-2 text-neutral-400 text-sm mb-1">
            ...
            <p className={`text-2xl font-bold ${overview.expiring_30_days > 0 ? 'text-amber-400' : 'text-white'}`}>
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-lg p-4">
            ...
            <p className={`text-2xl font-bold ${overview.expired_count > 0 ? 'text-red-400' : 'text-green-400'}`}>
      ...
      {expiring.length > 0 && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-medium text-amber-400">Certifications Expiring Soon</h3>
          </div>
          <div className="space-y-2">
            {expiring.slice(0, 5).map((item) => (
              <div key={item.record_id} className="flex items-center justify-between text-sm">
                <div>
                  ...
                  <span className="text-neutral-400"> - {item.course_name}</span>
                </div>
                <span className={item.is_expired ? 'text-red-400' : 'text-amber-400'}>
                  {item.is_expired 
            ...
            {expiring.length > 5 && (
              <Link href="/dashboard/people/training/expiring" className="text-amber-400 text-sm hover:underline">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-[#0B0D13] border border-white/[0.06] rounded-lg text-white placeholder-neutral-500 focus:outline-none focus:border-[#EC4899]"
          />
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                  course.is_mandatory ? 'bg-red-500/20' : 'bg-white/[0.05]'
                }`}>
                  <GraduationCap className={`w-6 h-6 ${course.is_mandatory ? 'text-red-400' : 'text-neutral-400'}`} />
                </div>
                <div>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="hidden md:flex items-center gap-4 text-sm">
                  <div className="text-center">
                    <p className="text-green-400 font-medium">{course.completed_valid}</p>
                    <p className="text-neutral-500 text-xs">Valid</p>
                  </div>
                  <div className="text-center">
                    <p className={`font-medium ${course.expiring_30_days > 0 ? 'text-amber-400' : 'text-neutral-400'}`}>
                      {course.expiring_30_days}
                    </p>
                    <p className="text-neutral-500 text-xs">Expiring</p>
                  </div>
                  <div className="text-center">
                    <p className={`font-medium ${course.expired > 0 ? 'text-red-400' : 'text-neutral-400'}`}>
                      {course.expired}
                    </p>
                    <p className="text-neutral-500 text-xs">Expired</p>
                  </div>
                </div>
                ...
                <div className="w-20 text-right">
                  ...
                  <p className="text-neutral-500 text-xs">Compliance</p>
                </div>
      ...
      {filteredStats.length === 0 && (
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-lg p-12 text-center">
          <GraduationCap className="w-12 h-12 text-neutral-500 mx-auto mb-4" />
