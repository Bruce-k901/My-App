...
export default function HolidayBalancesPage() {
  ...
  const getEmployeeTypeBadge = (type: string) => {
    switch (type) {
      ...
      default:
        return (
          <span className="px-2 py-1 bg-neutral-500/20 text-neutral-400 rounded-full text-xs">
            Unknown
          </span>
        );
    }
      ...
      <div className="bg-blue-500/10 border border-blue-500/50 rounded-lg p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-blue-300">
          ...
          <ul className="list-disc list-inside space-y-1.5 text-blue-300/90">
            ...
            <li><strong>Available days:</strong> Accrued + Carried Over + Adjustments - Taken - Pending</li>
            <li><strong>Days in lieu:</strong> Includes overtime conversions and manual adjustments</li>
            <li><strong>Overtime:</strong> Converted to holiday days (7.5 hours = 1 day)</li>
          </ul>
        </div>
      </div>
        ...
        <div className="bg-theme-button border border-theme rounded-lg p-8">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-white mb-2">No Leave Balances Found</h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-white/[0.05] border-b border-white/[0.06]">
                <tr>
                  {isManager && <th className="px-4 py-3 text-left text-sm font-medium text-neutral-300">Employee</th>}
                  <th className="px-4 py-3 text-left text-sm font-medium text-neutral-300">Leave Type</th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-neutral-300">Type</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-neutral-300">Entitled</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-neutral-300">Accrued</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-neutral-300">Carried Over</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-neutral-300">Days in Lieu</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-neutral-300">Taken</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-neutral-300">Pending</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-neutral-300">
                    <span className="text-green-400">Available</span>
                  </th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-neutral-300">Remaining</th>
                  {isManager && (
                    <th className="px-4 py-3 text-right text-sm font-medium text-neutral-300">Avg Hours (13w)</th>
                  )}
                </tr>
              </thead>
              <tbody className="divide-y divide-white/[0.06]">
                {balances.map((balance) => (
                  <tr key={balance.id} className="hover:bg-white/[0.02] transition-colors">
                    {isManager && (
                      <td className="px-4 py-3">
                        <div>
                          <p className="text-white font-medium text-sm">{balance.full_name}</p>
                          {balance.email && (
                            <p className="text-neutral-400 text-xs">{balance.email}</p>
                          )}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="text-white text-sm">{balance.leave_type_name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div>
                        <span className="text-white text-sm font-medium">{balance.entitled_days}</span>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {balance.overtime_holiday_days > 0 && (
                          <span className="text-purple-300 text-xs" title="From overtime">
