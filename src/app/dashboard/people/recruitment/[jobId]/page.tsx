...
Apply here: ${getPublicJobUrl()}
  ...
  if (!job) {
    return (
      <div className="p-6">
        <div className="text-theme-primary/60 text-center">Job not found</div>
      </div>
    )
  }
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-semibold text-theme-primary">{job.title}</h1>
 ...
 className="px-4 py-2 rounded-lg text-sm bg-gray-100 dark:bg-white/5 border border-theme text-theme-primary focus:outline-none focus:border-blue-500 dark:focus:border-blue-400 pr-10 appearance-none cursor-pointer disabled:opacity-50 [&>option]:bg-white dark:[&>option]:bg-[#1a1d24] [&>option]:text-theme-primary"
            ...
            <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none">
              <svg className="w-4 h-4 text-theme-primary/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {job.status === 'open' && (
            <button
              ...
              className="px-4 py-2 rounded-lg text-sm bg-theme-muted hover:bg-gray-200 dark:hover:bg-white/15 border border-theme text-theme-primary flex items-center gap-2"
            >
          ...
          <Link
            ...
            className="px-4 py-2 rounded-lg text-sm bg-transparent text-module-fg border border-blue-600 dark:border-blue-400 hover:shadow-module-glow dark:hover:shadow-module-glow transition-all flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-theme-surface border border-theme rounded-xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-theme-primary font-semibold text-lg">Job Overview</h2>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              {job.department && (
                <div className="flex items-center gap-3">
                  <Briefcase className="w-4 h-4 text-theme-primary/40" />
                  <div>
                    <div className="text-xs text-theme-primary/50">Department</div>
                    <div className="text-theme-primary text-sm">{job.department}</div>
                  </div>
                </div>
              )}
              ...
              {job.location && (
                <div className="flex items-center gap-3">
                  <MapPin className="w-4 h-4 text-theme-primary/40" />
                  <div>
                    <div className="text-xs text-theme-primary/50">Location</div>
                    <div className="text-theme-primary text-sm">{job.location}</div>
                  </div>
                </div>
              )}
              ...
              {(job.pay_rate_min || job.pay_rate_max) && (
                <div className="flex items-center gap-3">
                  <DollarSign className="w-4 h-4 text-theme-primary/40" />
                  <div>
                    <div className="text-xs text-theme-primary/50">Pay Range</div>
              ...
              {job.contract_hours && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs text-theme-primary/50">Contract Hours</div>
                    <div className="text-theme-primary text-sm">{job.contract_hours} hrs/week</div>
                  </div>
                </div>
              )}
            </div>
            ...
            {job.description && (
              <div className="pt-4 border-t border-theme">
                <h3 className="text-theme-primary font-medium mb-2">Description</h3>
                <p className="text-theme-primary/70 text-sm whitespace-pre-wrap">{job.description}</p>
              </div>
            )}
          </div>
          ...
          {(job.required_skills || job.required_certifications || job.experience_required) && (
            <div className="bg-theme-surface border border-theme rounded-xl p-6">
              <h2 className="text-theme-primary font-semibold mb-4">Requirements</h2>

              {job.required_skills && job.required_skills.length > 0 && (
                <div className="mb-4">
                  <h3 className="text-theme-primary/70 text-sm font-medium mb-2">Required Skills</h3>
                  <div className="flex flex-wrap gap-2">
                    {job.required_skills.map((skill, idx) => (
                      <span
                        key={idx}
                        className="px-2 py-1 text-xs rounded-full bg-gray-100 dark:bg-white/5 text-theme-primary/70 border border-theme"
                      >
                        {skill}
                      </span>
                    ))}
              ...
              {job.required_certifications && job.required_certifications.length > 0 && (
                <div className="mb-4">
                  <h3 className="text-theme-primary/70 text-sm font-medium mb-2">Required Certifications</h3>
                  <div className="flex flex-wrap gap-2">
              ...
              {job.experience_required && (
                <div>
                  <h3 className="text-theme-primary/70 text-sm font-medium mb-2">Experience</h3>
                  <p className="text-theme-primary/70 text-sm">{job.experience_required}</p>
                </div>
              )}
            </div>
          )}
          ...
          <div className="bg-theme-surface border border-theme rounded-xl p-6">
            ...
            {applicationCount === 0 ? (
              <div className="text-center py-8 text-theme-primary/40 text-sm">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-theme-surface border border-theme rounded-xl p-4">
            <h3 className="text-theme-primary font-semibold mb-3 text-sm">Share Job</h3>
            <div className="space-y-2">
              {job.status === 'open' ? (
                <>
                  <button
                    onClick={copyJobUrl}
                    className="w-full px-3 py-2 rounded-lg text-sm bg-transparent text-module-fg border border-blue-600 dark:border-blue-400 hover:shadow-module-glow dark:hover:shadow-module-glow transition-all text-left flex items-center gap-2 font-medium"
                  >
                  ...
                  <button
                    onClick={() => setShowShareModal(true)}
                    className="w-full px-3 py-2 rounded-lg text-sm bg-theme-muted hover:bg-gray-200 dark:hover:bg-white/15 border border-theme text-theme-primary text-left flex items-center gap-2"
                  >
                    <Share2 className="w-4 h-4" />
                ...
                <div className="text-theme-primary/40 text-xs py-2 text-center">
          ...
          {/* Job Insights */}
          <div className="bg-theme-surface border border-theme rounded-xl p-4">
            <h3 className="text-theme-primary font-semibold mb-3 text-sm">Job Insights</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-theme-primary/50">Total Applications</span>
                <span className="text-theme-primary font-semibold">{applicationCount}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-theme-primary/50">Posted</span>
                <span className="text-theme-primary/70 text-xs">
              ...
              {job.published_at && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-theme-primary/50">Published</span>
                  <span className="text-theme-primary/70 text-xs">
      ...
      {showShareModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#14161c] border border-theme rounded-xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-theme">
              <h2 className="text-xl font-semibold text-theme-primary">Share & Post Job</h2>
              <button
                onClick={() => setShowShareModal(false)}
                className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 text-theme-primary/60 hover:text-theme-primary"
              >
                ×
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              {/* Shareable Link */}
              <div>
                <h3 className="text-theme-primary font-medium mb-3 text-sm">Shareable Link</h3>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={getPublicJobUrl()}
                    readOnly
                    className="flex-1 px-3 py-2 bg-gray-100 dark:bg-white/5 border border-theme rounded-lg text-theme-primary/70 text-sm font-mono"
                  />
                  <button
                    onClick={copyJobUrl}
                    disabled={!job.is_published || job.status !== 'open'}
                    className="px-4 py-2 rounded-lg bg-transparent text-module-fg border border-blue-600 dark:border-blue-400 hover:shadow-module-glow dark:hover:shadow-module-glow transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:shadow-none"
                  >
                    <Copy className="w-4 h-4" />
                    Copy
                  </button>
                </div>
                <p className="text-xs text-theme-primary/50 mt-2">
              ...
              {/* Social Media */}
              <div>
                <h3 className="text-theme-primary font-medium mb-3 text-sm">Share on Social Media</h3>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { name: 'LinkedIn', key: 'linkedin', icon: '💼', color: 'bg-blue-500/10 hover:bg-module-fg/10 border-blue-500/30' },
                    { name: 'Twitter/X', key: 'twitter', icon: '🐦', color: 'bg-sky-500/10 hover:bg-sky-500/20 border-sky-500/30' },
                    { name: 'Facebook', key: 'facebook', icon: '📘', color: 'bg-blue-600/10 hover:bg-module-fg/10 border-blue-600/30' },
                    { name: 'WhatsApp', key: 'whatsapp', icon: '💬', color: 'bg-green-500/10 hover:bg-module-fg/10 border-green-500/30' },
                  ].map((social) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-theme-primary text-sm font-medium">{social.name}</span>
                      </button>
                    )
                  })}
                </div>
              ...
              {/* Job Boards */}
              <div>
                <h3 className="text-theme-primary font-medium mb-3 text-sm">Post to Job Boards</h3>
                <p className="text-xs text-theme-primary/60 mb-3">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <a
                      ...
                      className="flex items-center justify-between p-3 rounded-lg bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 border border-theme text-theme-primary transition-all"
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-xl">{board.icon}</span>
                        <span className="font-medium text-sm">{board.name}</span>
                      </div>
                      <ExternalLink className="w-4 h-4 text-theme-primary/40" />
                    </a>
                  ))}
                </div>
              </div>
