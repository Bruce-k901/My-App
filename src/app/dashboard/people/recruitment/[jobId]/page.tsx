...
Apply here: ${getPublicJobUrl()}
  ...
  if (!job) {
    return (
      <div className="p-6">
        <div className="text-white/60 text-center">Job not found</div>
      </div>
    )
  }
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <select
              ...
              className="px-4 py-2 rounded-lg text-sm bg-white/5 border border-white/10 text-white focus:outline-none focus:border-[#EC4899] pr-10 appearance-none cursor-pointer disabled:opacity-50 [&>option]:bg-[#1a1d24] [&>option]:text-white"
            >
          ...
          {job.status === 'open' && (
            <button
              ...
              className="px-4 py-2 rounded-lg text-sm bg-white/10 hover:bg-white/15 border border-white/10 text-white flex items-center gap-2"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-white font-semibold text-lg">Job Overview</h2>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              {job.department && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs text-white/50">Department</div>
                    <div className="text-white text-sm">{job.department}</div>
                  </div>
                </div>
              )}
              ...
              {job.location && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-white text-sm">{job.location}</div>
                  </div>
                </div>
              )}
              ...
              {(job.pay_rate_min || job.pay_rate_max) && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs text-white/50">Pay Range</div>
              ...
              {job.contract_hours && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-white text-sm">{job.contract_hours} hrs/week</div>
                  </div>
                </div>
              )}
            </div>
            ...
            {job.description && (
              <div className="pt-4 border-t border-white/[0.06]">
                ...
                <p className="text-white/70 text-sm whitespace-pre-wrap">{job.description}</p>
              </div>
            )}
          </div>
          ...
          {(job.required_skills || job.required_certifications || job.experience_required) && (
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
              ...
              {job.required_skills && job.required_skills.length > 0 && (
                <div className="mb-4">
                  <h3 className="text-white/70 text-sm font-medium mb-2">Required Skills</h3>
              ...
              {job.required_certifications && job.required_certifications.length > 0 && (
                <div className="mb-4">
                  <h3 className="text-white/70 text-sm font-medium mb-2">Required Certifications</h3>
                  <div className="flex flex-wrap gap-2">
              ...
              {job.experience_required && (
                <div>
                  <h3 className="text-white/70 text-sm font-medium mb-2">Experience</h3>
                  <p className="text-white/70 text-sm">{job.experience_required}</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4">
            <h3 className="text-white font-semibold mb-3 text-sm">Share Job</h3>
            <div className="space-y-2">
              {job.status === 'open' ? (
                <>
                  <button
                    onClick={copyJobUrl}
                    className="w-full px-3 py-2 rounded-lg text-sm bg-transparent text-[#EC4899] border border-[#EC4899] hover:shadow-[0_0_12px_rgba(236,72,153,0.7)] transition-all text-left flex items-center gap-2 font-medium"
                  >
                  ...
                  <button
                    onClick={() => setShowShareModal(true)}
                    className="w-full px-3 py-2 rounded-lg text-sm bg-white/10 hover:bg-white/15 border border-white/10 text-white text-left flex items-center gap-2"
                  >
                    <Share2 className="w-4 h-4" />
                ...
                <div className="text-white/40 text-xs py-2 text-center">
          ...
          {/* Job Insights */}
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4">
            <h3 className="text-white font-semibold mb-3 text-sm">Job Insights</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-white/50">Total Applications</span>
      ...
      {showShareModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#14161c] border border-white/10 rounded-xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-white/[0.06]">
              <h2 className="text-xl font-semibold text-white">Share & Post Job</h2>
              <button
            ...
            <div className="p-6 space-y-6">
              ...
              {/* Shareable Link */}
              <div>
                <h3 className="text-white font-medium mb-3 text-sm">Shareable Link</h3>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={getPublicJobUrl()}
                    readOnly
                    className="flex-1 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white/70 text-sm font-mono"
                  />
                  <button
                    onClick={copyJobUrl}
                    disabled={!job.is_published || job.status !== 'open'}
                    className="px-4 py-2 rounded-lg bg-transparent text-[#EC4899] border border-[#EC4899] hover:shadow-[0_0_12px_rgba(236,72,153,0.7)] transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:shadow-none"
                  >
                    <Copy className="w-4 h-4" />
                    Copy
                  </button>
              ...
              {/* Social Media */}
              <div>
                <h3 className="text-white font-medium mb-3 text-sm">Share on Social Media</h3>
              ...
              {/* Job Boards */}
              <div>
                <h3 className="text-white font-medium mb-3 text-sm">Post to Job Boards</h3>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <a
                      ...
                      className="flex items-center justify-between p-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-white transition-all"
                    >
                      <div className="flex items-center gap-3">
