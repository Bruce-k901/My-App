...
export default function ConfirmationsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-white mb-2">Confirmation Responses</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-12 text-center">
          ...
          <div className="text-white font-semibold text-lg">No responses yet</div>
          ...
          {responses.map((response) => (
            <div
              ...
              className={`bg-white/[0.03] border rounded-xl p-5 ${
                response.processed ? 'border-white/[0.03] opacity-60' : 'border-white/[0.06]'
              }`}
            >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <Link
                      ...
                      className="text-white font-semibold hover:text-[#EC4899] transition-colors"
                    >
              ...
              {!response.processed && (
                <div className="flex items-center gap-3">
                  <Link
                    href={`/dashboard/people/recruitment/candidates/${(response.candidate as any).id}`}
                    className="px-3 py-1.5 text-xs rounded-lg bg-[#EC4899]/10 text-[#EC4899] border border-[#EC4899]/30 hover:bg-[#EC4899]/20"
                  >
                    View Candidate
                  </Link>
                  <button
                    onClick={() => markProcessed(response.id)}
                    className="px-3 py-1.5 text-xs rounded-lg bg-white/5 text-white/60 border border-white/10 hover:bg-white/10"
                  >
                    Mark as Processed
                  </button>
                </div>
