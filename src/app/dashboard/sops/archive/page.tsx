...
export default function SOPsArchivePage() {
 ...
 className="p-2 rounded-lg bg-[rgb(var(--surface-elevated))] hover:bg-theme-button text-[rgb(var(--text-tertiary))] dark:text-theme-tertiary hover:text-[rgb(var(--text-primary))] transition-colors"
        >
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-[rgb(var(--text-primary))] dark:text-white">Archived SOPs</h1>
          <p className="text-[rgb(var(--text-secondary))] dark:text-theme-tertiary text-sm mt-1">View and restore archived SOPs</p>
        </div>
      </div>
 ...
 className="w-full pl-10 pr-4 py-2 bg-[rgb(var(--surface-elevated))] dark:bg-white/5 border border-[rgb(var(--border))] dark:border-white/10 rounded-lg text-[rgb(var(--text-primary))] placeholder:text-[rgb(var(--text-tertiary))] dark:placeholder:text-theme-tertiary focus:outline-none focus:border-module-fg dark:focus:border-module-fg"
        />
        <Archive className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-[rgb(var(--text-tertiary))] dark:text-theme-tertiary" />
      </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[rgb(var(--border))] dark:border-white/[0.06]">
                  <th className="px-4 py-3 text-left text-sm font-medium text-[rgb(var(--text-secondary))] dark:text-theme-tertiary">Title</th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-[rgb(var(--text-secondary))] dark:text-theme-tertiary">Type</th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-[rgb(var(--text-secondary))] dark:text-theme-tertiary">Version</th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-[rgb(var(--text-secondary))] dark:text-theme-tertiary">Archived</th>
                  <th className="px-4 py-3 text-right text-sm font-medium text-[rgb(var(--text-secondary))] dark:text-theme-tertiary">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredSOPs.map((sop) => (
                  <tr key={sop.id} className="border-b border-[rgb(var(--border))] dark:border-white/[0.03] hover:bg-black/[0.02] dark:hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-3 text-[rgb(var(--text-secondary))] dark:text-theme-tertiary text-sm">{sop.sop_type || '-'}</td>
                    <td className="px-4 py-3 text-[rgb(var(--text-secondary))] dark:text-theme-tertiary text-sm">v{sop.version_number || '1.0'}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        ...
                        className="px-3 py-1.5 bg-module-fg/10 dark:bg-module-fg/10 hover:bg-module-fg/20 dark:hover:bg-module-fg/20 text-module-fg dark:text-module-fg border border-module-fg/30 dark:border-module-fg/30 rounded-lg transition-colors text-sm"
                      >
                        Restore
                      </button>
                    </td>
