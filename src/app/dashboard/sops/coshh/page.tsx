...
import { Search, Upload, Download, FileText, X, CheckCircle, AlertTriangle, RefreshCw, ExternalLink } from '@/components/ui/icons';
...
export default function COSHHPage() {
  ...
  const hasCOSHHSheet = (chemicalId: string): boolean => {
    const sheet = getCOSHHSheetForChemical(chemicalId);
    // Sheet exists AND has a file uploaded (file_url is not null)
    return !!sheet && !!sheet.file_url;
  };
  ...
  const handleUpload = async (chemical: Chemical) => {
    ...
    try {
      ...
      // For private buckets, we store the file path and generate signed URLs on demand
      ...
      // Update chemicals_library with the file path (for reference)
      const { error: updateChemicalError } = await supabase
        .from('chemicals_library')
        .update({ coshh_sheet_url: filePath })
        .eq('id', chemical.id);
  ...
  const handleDownload = async (sheet: COSHHSheet) => {
    try {
      // For private buckets, we need to generate a signed URL
      // The file_url stored in the database is the file path
      const filePath = sheet.file_url;
      ...
      console.error('Error downloading COSHH sheet:', error);
      showToast({
        title: 'Error opening file',
        description: (error as Error).message,
        type: 'error'
  ...
  const stats = {
    ...
    withSheets: chemicals.filter(c => hasCOSHHSheet(c.id)).length,
    withoutSheets: chemicals.filter(c => !hasCOSHHSheet(c.id)).length
  };
 ...
 <div className="text-gray-600 dark:text-theme-tertiary text-sm">Missing Sheets</div>
      ...
      {stats.withoutSheets > 0 && (
        <div className="bg-orange-50 dark:bg-orange-500/10 border border-orange-200 dark:border-orange-500/30 rounded-xl p-4 flex items-center gap-3">
          <AlertTriangle size={20} className="text-orange-600 dark:text-orange-400" />
          <div className="flex-1">
            <div className="text-orange-800 dark:text-orange-400 font-semibold">
 ...
 <p className="text-gray-600 dark:text-theme-tertiary">
        ...
        <div className="space-y-3">
          {filteredChemicals.map((chemical) => {
            ...
            const hasSheet = hasCOSHHSheet(chemical.id);
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {hasSheet ? (
                        ...
                        <span className="px-2 py-1 bg-orange-50 dark:bg-orange-500/20 text-orange-700 dark:text-orange-400 border border-orange-200 dark:border-orange-500/40 rounded text-xs flex items-center gap-1">
