...
import { Plus, Search, Upload, Download, Edit, Trash2, Save, X, ChevronDown, ChevronRight, Heart, Package } from '@/components/ui/icons';
...
export default function FirstAidLibraryPage() {
  ...
  const saveRow = async (id: string) => {
    ...
    try {
      ...
      if (supplierVal && companyId) {
        await ensureSupplierExists(supplierVal, companyId, { sourceLibrary: 'first_aid' });
      }
  ...
  const uniqueSuppliers = useMemo(() => {
    ...
    firstAidItems.forEach((item: any) => {
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-theme-secondary">Manage first aid supplies and equipment</p>
          </div>
 ...
 className="bg-theme-surface ] border border-theme rounded-lg px-4 py-2.5 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500 min-w-[180px] appearance-none cursor-pointer"
        ...
        {loading ? (
          <div className="text-theme-secondary text-center py-8">Loading first aid supplies...</div>
        ) : filteredItems.length === 0 ? (
          <div className="bg-theme-surface border border-theme rounded-xl p-8 text-center">
            <p className="text-theme-secondary">No first aid supplies found.</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-theme-button border-b border-theme">
                <tr>
                  ...
                  <th className="text-left px-2 py-3 font-semibold text-gray-900 dark:text-module-fg text-[0.95rem]">Type</th>
                </tr>
              </thead>
              <tbody>
 ...
 <input className="w-full bg-theme-surface ] border border-theme rounded px-2 py-1 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500"value={rowDraft?.item_name ??''} onChange={(e) => setRowDraft((d: any) => ({ ...d, item_name: e.target.value }))} />
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {isGlobal && <span className="text-xs text-theme-secondary bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded">Global</span>}
                            </div>
                          )}
                        </td>
 ...
 <select className="w-full bg-theme-surface ] border border-theme rounded px-2 py-1 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500"value={rowDraft?.category ??''} onChange={(e) => setRowDraft((d: any) => ({ ...d, category: e.target.value }))}>
                        ...
                        <td className="px-2 py-3 text-theme-secondary text-sm whitespace-nowrap">
                          {editingRowId === item.id ? (
 <input className="w-full bg-theme-surface ] border border-theme rounded px-2 py-1 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500"value={rowDraft?.pack_size ??''} onChange={(e) => setRowDraft((d: any) => ({ ...d, pack_size: e.target.value }))} />
                          ) : (
                            item.pack_size || '-'
                          )}
                        </td>
                        ...
                        <td colSpan={5} className="px-4 py-4 bg-gray-50 dark:bg-white/[0.02]">
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div className="bg-theme-surface border border-theme rounded-lg p-3">
                              <div className="text-xs text-theme-tertiary mb-1">Sub Category</div>
                              {editingRowId === item.id ? (
 <input className="w-full bg-theme-surface ] border border-theme rounded px-2 py-1 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500"value={rowDraft?.sub_category ??''} onChange={(e) => setRowDraft((d: any) => ({ ...d, sub_category: e.target.value }))} />
                              ) : (
                                <div className="text-sm text-theme-primary font-medium">{item.sub_category || '-'}</div>
                              )}
                            </div>
                            <div className="bg-theme-surface border border-theme rounded-lg p-3">
 ...
 <input className="w-full bg-theme-surface ] border border-theme rounded px-2 py-1 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500"value={rowDraft?.standard_compliance ??''} onChange={(e) => setRowDraft((d: any) => ({ ...d, standard_compliance: e.target.value }))} />
                            ...
                            <div className="bg-theme-surface border border-theme rounded-lg p-3">
                              <div className="text-xs text-theme-tertiary mb-1">Expiry Period (Months)</div>
                              {editingRowId === item.id ? (
 <input type="number"className="w-full bg-theme-surface ] border border-theme rounded px-2 py-1 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500"value={rowDraft?.expiry_period_months ??''} onChange={(e) => setRowDraft((d: any) => ({ ...d, expiry_period_months: e.target.value }))} />
                              ) : (
                                <div className="text-sm text-theme-primary font-medium">{item.expiry_period_months ? `${item.expiry_period_months} months` : '-'}</div>
                              )}
                            </div>
                            <div className="bg-theme-surface border border-theme rounded-lg p-3">
 ...
 <input type="number"step="0.01"className="w-full bg-theme-surface ] border border-theme rounded px-2 py-1 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500"value={rowDraft?.unit_cost ??''} onChange={(e) => setRowDraft((d: any) => ({ ...d, unit_cost: e.target.value }))} />
                            ...
                            <div className="bg-theme-surface border border-theme rounded-lg p-3">
                              <div className="text-xs text-theme-tertiary mb-1">Storage Requirements</div>
                              {editingRowId === item.id ? (
 <input className="w-full bg-theme-surface ] border border-theme rounded px-2 py-1 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500"value={rowDraft?.storage_requirements ??''} onChange={(e) => setRowDraft((d: any) => ({ ...d, storage_requirements: e.target.value }))} />
                              ) : (
                            ...
                            <div className="bg-theme-surface border border-theme rounded-lg p-3 md:col-span-2 lg:col-span-3">
                              <div className="text-xs text-theme-tertiary mb-1">Typical Usage</div>
                              {editingRowId === item.id ? (
 <input className="w-full bg-theme-surface ] border border-theme rounded px-2 py-1 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500"value={rowDraft?.typical_usage ??''} onChange={(e) => setRowDraft((d: any) => ({ ...d, typical_usage: e.target.value }))} />
                              ) : (
                                <div className="text-sm text-theme-primary font-medium">{item.typical_usage || '-'}</div>
                              )}
                            </div>
                            <div className="bg-theme-surface border border-theme rounded-lg p-3 md:col-span-2 lg:col-span-3">
 ...
 <button onClick={() => cancelEdit(item.id)} className="px-3 py-2 rounded-lg border border-gray-300 dark:border-neutral-600 text-theme-secondary bg-theme-surface ] hover:bg-theme-muted transition flex items-center gap-2">
                              ...
                              <>
                                {!isGlobal && (
                                  <>
 <button aria-label="Edit First Aid Supply"onClick={() => handleEdit(item)} className="h-9 w-9 inline-flex items-center justify-center rounded-lg border border-emerald-600 dark:border-module-fg/30 text-module-fg bg-theme-surface ] hover:bg-theme-muted hover:shadow-module-glow transition">
                                      <Edit size={16} />
 ...
 <button aria-label="Delete First Aid Supply"onClick={() => handleDelete(item.id)} className="h-9 w-9 inline-flex items-center justify-center rounded-lg border border-red-600 dark:border-red-500/60 text-red-600 dark:text-red-400 bg-theme-surface ] hover:bg-red-50 dark:hover:bg-red-500/10 hover:shadow-module-glow transition">
                                ...
                                {isGlobal && (
                                  <span className="text-xs text-theme-tertiary">Global items are read-only</span>
                                )}
                              </>
                            )}
                          </div>
