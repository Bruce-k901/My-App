...
export default function DisposablesLibraryPage() {
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-theme-secondary">Manage disposable items and packaging</p>
          </div>
 ...
 className="bg-theme-surface ] border border-theme rounded-lg px-4 py-2.5 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500 min-w-[180px] appearance-none cursor-pointer"
        ...
        {loading ? (
          <div className="text-theme-secondary text-center py-8">Loading disposables...</div>
        ) : filteredItems.length === 0 ? (
          <div className="bg-theme-surface border border-theme rounded-xl p-8 text-center">
            <p className="text-theme-secondary">No disposables found.</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-theme-button border-b border-theme">
                <tr>
                  ...
                  <th className="text-left px-2 py-3 font-semibold text-gray-900 dark:text-module-fg text-[0.95rem]">Material</th>
                </tr>
              </thead>
              <tbody>
 ...
 <input type="number"className="w-full bg-theme-surface ] border border-theme rounded px-2 py-1 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500"value={rowDraft?.reorder_level ??''} onChange={(e) => setRowDraft((d: any) => ({ ...d, reorder_level: e.target.value }))} />
                            ...
                            <div className="bg-theme-surface border border-theme rounded-lg p-3">
                              <div className="text-xs text-theme-tertiary mb-1">Storage Location</div>
                              {editingRowId === item.id ? (
 <input className="w-full bg-theme-surface ] border border-theme rounded px-2 py-1 text-theme-primary focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-500"value={rowDraft?.storage_location ??''} onChange={(e) => setRowDraft((d: any) => ({ ...d, storage_location: e.target.value }))} />
                              ) : (
                                <div className="text-sm text-theme-primary font-medium">{item.storage_location || '-'}</div>
                              )}
                            </div>
                            <div className="bg-theme-surface border border-theme rounded-lg p-3">
 ...
 <button onClick={() => cancelEdit(item.id)} className="px-3 py-2 rounded-lg border border-gray-300 dark:border-neutral-600 text-theme-secondary bg-theme-surface ] hover:bg-theme-muted transition flex items-center gap-2">
                              ...
                              <>
 <button aria-label="Edit Disposable"onClick={() => handleEdit(item)} className="h-9 w-9 inline-flex items-center justify-center rounded-lg border border-emerald-600 dark:border-module-fg/30 text-module-fg bg-theme-surface ] hover:bg-theme-muted hover:shadow-module-glow transition">
                                  <Edit size={16} />
