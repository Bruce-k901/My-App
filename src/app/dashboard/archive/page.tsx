...
export default function ArchiveCenterPage() {
 ...
 return <IconComponent className={`w-5 h-5 ${tab?.color ||'text-theme-secondary'} ${tab?.darkColor ||''}`} />
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-theme-primary">Archive Center</h1>
          </div>
 ...
 :'text-theme-secondary hover:text-theme-primary hover:bg-neutral-100 dark:hover:bg-white/[0.05]'
                  }`}
                >
        ...
        {/* Filters */}
        <div className="bg-theme-surface rounded-lg p-4 mb-6 border border-neutral-200 dark:border-white/[0.06]">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="md:col-span-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-neutral-50 dark:bg-white/[0.06] border border-neutral-200 dark:border-white/[0.1] rounded-lg text-theme-primary text-sm placeholder:text-theme-tertiary focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
            </div>
            ...
            <div>
              <select
                ...
                className="w-full px-3 py-2 bg-neutral-50 dark:bg-white/[0.06] border border-neutral-200 dark:border-white/[0.1] rounded-lg text-theme-primary text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
        ...
        {/* Results */}
        {loading ? (
          <div className="bg-theme-surface rounded-lg p-12 border border-neutral-200 dark:border-white/[0.06] flex items-center justify-center">
            <div className="flex flex-col items-center gap-3">
              <Loader2 className="w-8 h-8 text-blue-600 dark:text-blue-400 animate-spin" />
              <p className="text-theme-secondary">Loading archives...</p>
            </div>
          </div>
        ) : error ? (
          ...
          <div className="bg-theme-surface rounded-lg p-12 border border-neutral-200 dark:border-white/[0.06] text-center">
            <Archive className="w-12 h-12 text-neutral-400 dark:text-neutral-600 mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-theme-primary mb-2">No Archived Items</h3>
            <p className="text-theme-secondary">
              {searchQuery
          ...
          <div className="bg-theme-surface rounded-lg border border-neutral-200 dark:border-white/[0.06] overflow-hidden">

            {/* Table Header */}
            <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-neutral-50 dark:bg-white/[0.03] border-b border-neutral-200 dark:border-white/[0.06] text-sm font-medium text-theme-secondary">
              ...
              {filteredItems.map((item) => (
                ...
                >
                  ...
                  <div className="col-span-5">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-theme-primary font-medium truncate">{item.name}</div>
                        {item.category && (
                          <div className="text-xs text-theme-tertiary mt-0.5">{item.category}</div>
                        )}
                        {item.changes_summary && (
                          <div className="text-xs text-theme-secondary mt-1 line-clamp-2">
                            {item.changes_summary}
                          </div>
                        )}
                      </div>
                  ...
                  <div className="col-span-1 flex items-center">
                    {item.version && (
                      <span className="px-2 py-1 text-xs rounded bg-neutral-100 dark:bg-white/[0.06] text-theme-secondary">
                        {item.version}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="col-span-2 flex items-center justify-end gap-2">
                    ...
                    {item.can_restore && (
                      <button
                        ...
                        className="p-2 hover:bg-module-fg/10 rounded-lg transition-colors"
                        title="Restore"
                      >
        ...
        <div className="fixed inset-0 bg-black/40 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gradient-to-br dark:from-neutral-900 dark:to-neutral-950 rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden border border-neutral-200 dark:border-white/[0.08]">
            <div className="flex items-center justify-between p-6 border-b border-neutral-200 dark:border-white/[0.08]">
              <h3 className="text-lg font-semibold text-theme-primary">{selectedItem.name}</h3>
              <button
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-8rem)]">
              <div className="space-y-4">
                <div>
                  <div className="text-sm text-theme-tertiary mb-1">Type</div>
                  <div className="text-theme-primary capitalize">{selectedItem.type.replace('_', ' ')}</div>
                </div>

                {selectedItem.version && (
                  <div>
                    <div className="text-sm text-theme-tertiary mb-1">Version</div>
                    <div className="text-theme-primary">{selectedItem.version}</div>
                  </div>
                )}
                ...
                {selectedItem.category && (
                  <div>
                    ...
                    <div className="text-theme-primary">{selectedItem.category}</div>
                  </div>
                )}

                {selectedItem.changes_summary && (
                  <div>
                    <div className="text-sm text-theme-tertiary mb-1">Changes</div>
                    <div className="text-theme-primary">{selectedItem.changes_summary}</div>
                  </div>
                )}

                <div>
                  <div className="text-sm text-theme-tertiary mb-1">Archived</div>
                ...
                <div>
                  <div className="text-sm text-theme-tertiary mb-1">Archived By</div>
                  <div className="text-theme-primary">{selectedItem.archived_by_name || 'Unknown'}</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 p-6 border-t border-neutral-200 dark:border-white/[0.08]">
              <button
                onClick={() => setSelectedItem(null)}
                className="px-4 py-2 text-sm font-medium text-theme-secondary hover:text-theme-primary hover:bg-neutral-100 dark:hover:bg-white/[0.06] rounded-lg transition-colors"
              >
                Close
              </button>
              {selectedItem.can_restore && (
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium text-white bg-module-fg hover:bg-module-fg/90 rounded-lg transition-colors flex items-center gap-2"
                >
