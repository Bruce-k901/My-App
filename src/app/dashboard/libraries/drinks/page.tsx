...
export default function DrinksLibraryPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-semibold text-theme-primary">Drinks Library</h1>
              <p className="text-sm text-theme-tertiary">Manage spirits, mixers, and garnishes</p>
            </div>
          </div>
        </div>
      ...
      {loading ? (
        <div className="text-theme-tertiary text-center py-8">Loading drinks...</div>
      ) : filteredItems.length === 0 ? (
        <div className="bg-neutral-800/50 rounded-xl p-8 text-center border border-theme">
          <p className="text-theme-tertiary">No drinks found.</p>
        </div>
      ) : (
                ...
                return (
                  <React.Fragment key={item.id}>
                    <tr className="border-t border-theme hover:bg-neutral-800/50">
                      ...
                      <td className="px-2 py-3 text-theme-tertiary text-sm whitespace-nowrap">
                        {editingRowId === item.id ? (
                          <input type="number" step="0.1" className="w-full bg-neutral-800 border border-neutral-600 rounded px-2 py-1 text-theme-primary" value={rowDraft?.abv ?? ''} onChange={(e) => setRowDraft((d: any) => ({ ...d, abv: e.target.value }))} placeholder="%" />
                        ) : (
                          item.abv ? `${item.abv}%` : '-'
                        )}
                      </td>
                        ...
                        <td colSpan={4} className="px-4 py-4 bg-neutral-900/40">
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div className="bg-neutral-800/60 border border-theme rounded-lg p-3">
                              ...
                              {editingRowId === item.id ? (
                                <input className="w-full bg-neutral-800 border border-neutral-600 rounded px-2 py-1 text-theme-primary" value={rowDraft?.sub_category ?? ''} onChange={(e) => setRowDraft((d: any) => ({ ...d, sub_category: e.target.value }))} />
                              ) : (
                                <div className="text-sm text-theme-primary">{item.sub_category || '-'}</div>
                              )}
                            </div>
                            <div className="bg-neutral-800/60 border border-theme rounded-lg p-3">
                              <div className="text-xs text-theme-tertiary">Unit</div>
                              {editingRowId === item.id ? (
                                <input className="w-full bg-neutral-800 border border-neutral-600 rounded px-2 py-1 text-theme-primary" value={rowDraft?.unit ?? ''} onChange={(e) => setRowDraft((d: any) => ({ ...d, unit: e.target.value }))} placeholder="e.g., L, ml, bottle" />
                              ) : (
                                <div className="text-sm text-theme-primary">{item.unit || '-'}</div>
                              )}
                            </div>
                            <div className="bg-neutral-800/60 border border-theme rounded-lg p-3">
                            ...
                            <div className="bg-neutral-800/60 border border-theme rounded-lg p-3">
                              ...
                              {editingRowId === item.id ? (
                                <input className="w-full bg-neutral-800 border border-neutral-600 rounded px-2 py-1 text-theme-primary" value={rowDraft?.pack_size ?? ''} onChange={(e) => setRowDraft((d: any) => ({ ...d, pack_size: e.target.value }))} />
                              ) : (
                                <div className="text-sm text-theme-primary">{item.pack_size || '-'}</div>
                              )}
                            </div>
                            <div className="bg-neutral-800/60 border border-theme rounded-lg p-3">
                              <div className="text-xs text-theme-tertiary">Storage Type</div>
                              {editingRowId === item.id ? (
                                <input className="w-full bg-neutral-800 border border-neutral-600 rounded px-2 py-1 text-theme-primary" value={rowDraft?.storage_type ?? ''} onChange={(e) => setRowDraft((d: any) => ({ ...d, storage_type: e.target.value }))} />
                              ) : (
                                <div className="text-sm text-theme-primary">{item.storage_type || '-'}</div>
                              )}
                            </div>
                            <div className="bg-neutral-800/60 border border-theme rounded-lg p-3">
                              <div className="text-xs text-theme-tertiary">Shelf Life</div>
                              {editingRowId === item.id ? (
                                <input className="w-full bg-neutral-800 border border-neutral-600 rounded px-2 py-1 text-theme-primary" value={rowDraft?.shelf_life ?? ''} onChange={(e) => setRowDraft((d: any) => ({ ...d, shelf_life: e.target.value }))} />
                              ) : (
                                <div className="text-sm text-theme-primary">{item.shelf_life || '-'}</div>
                              )}
                            </div>
                            <div className="bg-neutral-800/60 border border-theme rounded-lg p-3">
                              <div className="text-xs text-theme-tertiary">Allergens</div>
                              {editingRowId === item.id ? (
                                <input className="w-full bg-neutral-800 border border-neutral-600 rounded px-2 py-1 text-theme-primary" placeholder="Comma or semicolon separated" value={(rowDraft?.allergens || []).join(', ')} onChange={(e) => setRowDraft((d: any) => ({ ...d, allergens: e.target.value.split(/[,;]/).map(s => s.trim()).filter(Boolean) }))} />
                              ) : (
                                <div className="text-sm text-theme-primary">{(item.allergens || []).join(', ') || '-'}</div>
                              )}
                            </div>
                          ...
                          <div className="flex items-center gap-2 mt-4">
                            {editingRowId === item.id ? (
                              ...
                              <>
                                <button aria-label="Edit Drink" onClick={() => handleEdit(item)} className="h-9 w-9 inline-flex items-center justify-center rounded-lg border border-magenta-500/60 text-magenta-400 bg-white/5 backdrop-blur-sm hover:bg-white/10 hover:border-magenta-400 hover:shadow-module-glow transition">
                                  <Edit size={16} />
                                  <span className="sr-only">Edit</span>
                                </button>
                                <button aria-label="Delete Drink" onClick={() => handleDelete(item.id)} className="h-9 w-9 inline-flex items-center justify-center rounded-lg border border-red-500/60 text-red-400 bg-white/5 backdrop-blur-sm hover:bg-white/10 hover:border-red-400 hover:shadow-module-glow transition">
                                  <Trash2 size={16} />
