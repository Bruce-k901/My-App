    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-4 py-2 border border-magenta-500/60 text-magenta-400 bg-white/5 backdrop-blur-sm hover:bg-white/10 hover:border-magenta-400 hover:shadow-[0_0_14px_rgba(233,0,126,0.55)] transition rounded-lg flex items-center gap-2"
        >
          <Package size={16} />
          New Request
        </button>
      ...
      {requests.length === 0 ? (
        <div className="bg-neutral-800/50 rounded-xl p-12 text-center border border-neutral-700">
          ...
          <p className="text-neutral-400 mb-4">No library requests yet</p>
          <button
            onClick={() => router.push('/dashboard/libraries/create')}
            className="px-4 py-2 border border-magenta-500/60 text-magenta-400 bg-white/5 backdrop-blur-sm hover:bg-white/10 hover:border-magenta-400 hover:shadow-[0_0_14px_rgba(233,0,126,0.55)] transition rounded-lg"
          >
        ...
        <div className="space-y-4">
          {requests.map((request) => (
            <div key={request.id} className="bg-neutral-800/50 rounded-xl border border-neutral-700 p-6">
              ...
              {(request.review_notes || request.rejection_reason || request.deployment_notes) && (
                <div className="mt-4 pt-4 border-t border-neutral-700">
                  {request.rejection_reason && (
                    <div className="mb-2">
                      <p className="text-xs text-neutral-400 mb-1">Rejection Reason:</p>
                      <p className="text-sm text-red-400">{request.rejection_reason}</p>
                    </div>
                  )}
                  {request.review_notes && (
                    <div className="mb-2">
                      <p className="text-xs text-neutral-400 mb-1">Review Notes:</p>
                      <p className="text-sm text-neutral-300">{request.review_notes}</p>
                    </div>
                  )}
                  {request.deployment_notes && (
                    <div>
                      <p className="text-xs text-neutral-400 mb-1">Deployment Notes:</p>
              ...
              {request.fields && Array.isArray(request.fields) && request.fields.length > 0 && (
                <div className="mt-4 pt-4 border-t border-neutral-700">
                  <p className="text-xs text-neutral-400 mb-2">Fields:</p>
