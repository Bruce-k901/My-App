...
import type { CompanyInfo, EHOReportData, SiteInfo } from "./types";
...
function safeData<T>(
  ...
  if (result.status !== "fulfilled") {
    console.warn(
      `[EHO Report] ${label || "Query"} rejected:`,
...
export async function fetchAllReportData(
  ...
  const { data: siteData } = await supabase
    .from("sites")
    .select(
      "id, name, address_line1, address_line2, city, postcode, company_id",
    )
    .eq("id", siteId)
    .single();
  ...
  const results = await Promise.allSettled([
    ...
    companyId
      ? supabase.from("companies").select(
        "id, name, legal_name, address_line1, city, postcode, contact_email, phone",
      ).eq("id", companyId).single()
      : Promise.resolve({ data: null, error: null }),
    // 1: Compliance summary
    supabase.rpc("get_compliance_summary", {
      p_site_id: siteId,
      p_start_date: startDate,
      p_end_date: endDate,
    }),
    // 2: Task completions (all categories)
    supabase.rpc("get_eho_report_data", {
      p_site_id: siteId,
      p_start_date: startDate,
      p_end_date: endDate,
      p_template_categories: null,
    }),
    // 3: Temperature records (direct query — RPC may not return data)
    supabase.from("temperature_logs")
      .select(
        "id, asset_id, reading, unit, recorded_at, status, profiles:recorded_by(full_name), asset:assets!asset_id(name, category)",
      )
      .eq("site_id", siteId)
    ...
    supabase.from("incidents").select(
      "id, title, description, incident_type, severity, incident_date, reported_date, reported_by, location, riddor_reportable, riddor_reported, riddor_reference, status, immediate_actions_taken, corrective_actions, investigation_notes, root_cause, casualties, emergency_services_called, photos",
    ).eq("site_id", siteId).or(
      `and(reported_date.gte.${startDate},reported_date.lte.${endDate}),and(incident_date.gte.${startDate},incident_date.lte.${endDate})`,
    ...
    // 9: Global documents
    companyId
      ? supabase.from("global_documents").select(
        "id, name, category, version, uploaded_at, expiry_date, is_active, notes, file_path",
      ).eq("company_id", companyId).eq("is_active", true).order("category")
      : Promise.resolve({ data: [], error: null }),
    // 10: COSHH data sheets
    companyId
      ? supabase.from("coshh_data_sheets").select(
        "id, product_name, manufacturer, hazard_types, document_type, issue_date, expiry_date, status, verification_status, file_url",
      ).eq("company_id", companyId).order("product_name")
      : Promise.resolve({ data: [], error: null }),
    // 11: Risk assessments
    companyId
      ? supabase.from("risk_assessments").select(
        "id, title, ref_code, template_type, assessor_name, assessment_date, next_review_date, status, total_hazards, hazards_controlled, highest_risk_level, linked_chemicals, linked_ppe",
      ).eq("company_id", companyId).order("template_type")
      : Promise.resolve({ data: [], error: null }),
    // 12: Assets
    supabase.from("assets").select(
      "id, name, category, brand, model, serial_number, status, install_date, last_service_date, next_service_date, warranty_end, ppm_status, ppm_frequency_months",
    ).eq("site_id", siteId).or("archived.is.null,archived.eq.false").order(
      "name",
    ),
    // 13: PAT appliances
    companyId
      ? supabase.from("pat_appliances").select(
        "id, name, brand, has_current_pat_label, purchase_date, notes",
      ).eq("site_id", siteId).eq("company_id", companyId).order("name")
      : Promise.resolve({ data: [], error: null }),
    // 14: Compliance scores
    supabase.from("site_compliance_score").select(
      "id, score, score_date, open_critical_incidents, overdue_corrective_actions, missed_daily_checklists, temperature_breaches_last_7d, breakdown",
    ).eq("site_id", siteId).gte("score_date", startDate).lte(
      "score_date",
      endDate,
    ...
    companyId
      ? supabase.from("profiles").select(
        "id, full_name, app_role, position_title, food_safety_level, food_safety_expiry_date, h_and_s_level, h_and_s_expiry_date, fire_marshal_trained, fire_marshal_expiry_date, first_aid_trained, first_aid_expiry_date, cossh_trained, cossh_expiry_date",
    ...
    // 18: Compliance matrix (full training status for all staff)
    companyId
      ? supabase.from("compliance_matrix_view").select(
  ...
  const temperatureRecords = rawTempLogs.map((log: any) => ({
    ...
    reading: typeof log.reading === "number"
      ? log.reading
      : parseFloat(log.reading),
