...
export function YesNoChecklistFeature({
  ...
  return (
    <div className="border-t border-theme pt-6">
      <h2 className="text-lg font-semibold text-theme-primary mb-4">Yes/No Checklist</h2>
      ...
      <div className="space-y-3">
        {items.map((item, index) => (
          <div key={index} className="bg-gray-50 dark:bg-white/[0.03] border border-theme rounded-lg p-4">
            <input
              type="text"
              value={item.text}
              onChange={(e) => updateItem(index, e.target.value)}
              placeholder="Enter question/item"
              className="w-full px-4 py-2 rounded-lg bg-white dark:bg-[#0f1220] border border-gray-300 dark:border-neutral-800 text-theme-primary focus:outline-none focus:ring-2 focus:ring-[#D37E91] focus:border-[#D37E91] mb-3"
            />
              ...
              <button
                ...
                className={`flex-1 px-4 py-2 rounded-lg border-2 transition-colors ${
                  item.answer === 'yes'
                    ...
                    : 'bg-white dark:bg-[#0f1220] border-gray-300 dark:border-neutral-800 text-theme-secondary hover:border-green-500/50'
                }`}
              >
                Yes
              </button>
              <button
                ...
                className={`flex-1 px-4 py-2 rounded-lg border-2 transition-colors ${
                  item.answer === 'no'
                    ...
                    : 'bg-white dark:bg-[#0f1220] border-gray-300 dark:border-neutral-800 text-theme-secondary hover:border-red-500/50'
                }`}
              >
                No
              </button>
