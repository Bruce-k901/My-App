...
export default function ScheduleTrialModal({ candidate, application, companyId, managerId, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await supabase.rpc('update_application_status', {
        ...
        p_notes: `Trial shift scheduled for ${date} at ${time || '09:00'}`,
      })

      // Update application with trial details
      const { error: updateError } = await supabase
        .from('applications')
        .update({
          trial_scheduled_at: new Date(`${date}T${time || '09:00'}`).toISOString(),
          trial_confirmation_status: 'pending', // Set to pending when scheduled
        })
      ...
      if (sendEmail) {
        const emailResponse = await fetch('/api/recruitment/send-trial-invite', {
          method: 'POST',
        ...
        if (emailResult.error) {
          toast.warning('Trial scheduled, but email notification failed')
        } else {
          toast.success('Trial shift scheduled and invitation sent!')
        }
      } else {
        toast.success('Trial shift scheduled!')
      }
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg text-theme-primary focus:outline-none focus:border-[#8A2B2B] [&>option]:bg-white dark:[&>option]:bg-[#1a1d24] [&>option]:text-theme-primary"
            >
