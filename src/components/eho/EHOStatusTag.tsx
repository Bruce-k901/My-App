...
export function EHOStatusTag({ status, count }: { status: Status; count: number }) {
  ...
  const color = status === "valid" ? "text-green-600 dark:text-green-400" : status === "due_review" ? "text-yellow-600 dark:text-yellow-400" : status === "expired" ? "text-red-600 dark:text-red-400" : "text-gray-600 dark:text-theme-secondary";
  ...
  return (
    ...
    >
      <span>{label}</span>
      <span className="text-gray-500 dark:text-theme-secondary">({count})</span>
    </span>
  );
}
