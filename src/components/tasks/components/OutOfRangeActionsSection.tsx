      ...
      {outOfRangeAssets.map((asset) => {
        ...
        return (
          <div key={asset.assetId} className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
            ...
            {hasAction && (
              <div className="flex items-center justify-between p-3 bg-white/[0.05] border border-white/[0.1] rounded-lg">
                <div className="flex items-center gap-2">
            ...
            {!hasAction && !isExpanded && (
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-3 py-2 bg-blue-500/20 border border-blue-500/50 rounded-lg text-blue-400 hover:bg-blue-500/30 transition-colors"
                >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-3 py-2 bg-red-500/20 border border-red-500/50 rounded-lg text-red-400 hover:bg-red-500/30 transition-colors"
                >
            ...
            {!hasAction && isExpanded && selectedAction === 'monitor' && (
              <div className="space-y-3 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  <Clock className="w-4 h-4 text-blue-400" />
                  <span className="text-sm font-medium text-blue-400">Schedule Monitoring</span>
                </div>
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-white/[0.03] border border-white/[0.1] rounded-lg text-white text-sm"
                  >
            ...
            {!hasAction && isExpanded && selectedAction === 'callout' && (
              <div className="space-y-3 p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  <Phone className="w-4 h-4 text-red-400" />
                  <span className="text-sm font-medium text-red-400">Request Callout</span>
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full px-3 py-2 bg-white/[0.03] border border-white/[0.1] rounded-lg text-white text-sm resize-none"
                  />
                </div>
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 px-3 py-2 bg-red-500 hover:bg-red-600 rounded-lg text-white text-sm font-medium transition-colors"
                  >
                    Confirm Callout
                  </button>
                  <button
      ...
      {outOfRangeAssets.length > 0 && (
        <p className="text-xs text-neutral-500">
          ...
          {placedActions.size < outOfRangeAssets.length && (
            <span className="text-red-400"> - All actions required to complete task</span>
          )}
        </p>
      )}
    </div>
