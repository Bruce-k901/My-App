...
export function ForecastInput({ date, currentForecast, onSave, onClose }: ForecastInputProps) {
  ...
  return (
    <div className="absolute top-full left-0 mt-1 bg-neutral-800 border border-theme rounded-lg shadow-xl z-50 p-3 min-w-[200px]">
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-medium text-theme-primary">Forecast Sales</span>
        <button
          onClick={onClose}
          className="p-0.5 hover:bg-neutral-700 rounded text-theme-tertiary hover:text-white"
        >
          <X className="w-3 h-3" />
        </button>
      </div>
      <form onSubmit={handleSubmit} className="space-y-2">
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-2 py-1 bg-neutral-700 hover:bg-neutral-600 text-theme-primary text-xs rounded transition-colors"
          >
            Cancel
          </button>
        </div>
