...
export function MetricsGrid({ tenantId, siteId }: Props) {
  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-[rgb(var(--surface))] dark:bg-white/[0.05] border border-[rgb(var(--border-hover))] dark:border-white/[0.1] rounded-xl p-4 md:p-5">
                ...
                {siteHistory.length ? (
                  siteHistory.slice(-14).map((entry) => (
                    <div
                      ...
                      className="flex-1 bg-black/10 dark:bg-white/10 rounded"
