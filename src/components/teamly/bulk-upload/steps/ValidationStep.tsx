...
import { CheckCircle, AlertTriangle, XCircle, Users, AlertCircle, Pencil, ChevronDown } from '@/components/ui/icons';
...
type FilterTab = 'all' | 'valid' | 'errors' | 'duplicates';
...
export function ValidationStep({
  ...
  const counts = useMemo(() => {
    ...
    const duplicates = rows.filter((r) => r._status === 'duplicate').length;
    const included = rows.filter((r) => r._included).length;
    return { valid, warnings, errors, duplicates, included, total: rows.length };
  }, [rows]);
  ...
  const filteredRows = useMemo(() => {
    switch (filter) {
      ...
      case 'duplicates':
        return rows.filter((r) => r._status === 'duplicate');
      default:
        return rows;
    }
  }, [rows, filter]);

  // Build ordered list of mapped fields for dynamic columns
  const visibleColumns = useMemo(() => {
    const mapped = columnMappings
      .filter((m) => m.targetField && !m.targetField.startsWith('_'))
      .map((m) => m.targetField!);

    const hasVirtualName = columnMappings.some(
      (m) => m.targetField === '_first_name' || m.targetField === '_last_name'
    );
    const hasFullName = mapped.includes('full_name');

    const result: Array<{ key: string; label: string }> = [];

    // full_name first (whether mapped directly or via virtual first/last)
    if (hasFullName || hasVirtualName) {
      result.push({ key: 'full_name', label: 'Name' });
    }

    for (const key of mapped) {
      if (key === 'full_name') continue;
      const field = TEAMLY_IMPORT_FIELDS.find((f) => f.key === key);
  ...
  const commitEdit = () => {
    ...
    import('@/lib/bulk-import/parser').then(({ validateRows }) => {
      import('@/lib/bulk-import/teamly-config').then(({ TEAMLY_IMPORT_FIELDS: fields }) => {
        const revalidated = validateRows(updated, fields);
  ...
  const renderCell = (row: ParsedRow, col: { key: string; label: string }) => {
    ...
    // Editable cells: full_name and email
    if (col.key === 'full_name' || col.key === 'email') {
        ...
        return (
          <td key={col.key} className="px-3 py-2 whitespace-nowrap">
            <input
              ...
              className="w-full px-1.5 py-0.5 text-xs bg-theme-surface border border-teamly-dark/50 dark:border-teamly/50 rounded focus:outline-none"
            />
          </td>
        );
      }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
        ...
        <div className="p-2.5 rounded-lg bg-blue-500/10 border border-blue-500/20 text-center">
          <p className="text-lg font-bold text-blue-500">{counts.duplicates}</p>
          <p className="text-[10px] text-blue-600 dark:text-blue-400 uppercase">Duplicates</p>
        </div>
      </div>

      {/* Filter tabs */}
      <div className="flex items-center gap-1 border-b border-theme">
        {(['all', 'valid', 'errors', 'duplicates'] as FilterTab[]).map((tab) => (
          <button
            key={tab}
            type="button"
            ...
            className={`px-3 py-2 text-xs font-medium border-b-2 transition-colors ${
              filter === tab
                ? 'border-teamly-dark dark:border-teamly text-teamly-dark dark:text-teamly'
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-3 flex-1 p-2.5 rounded-lg bg-theme-surface border border-theme">
          ...
          <div className="relative flex-1">
            <select
              ...
              className="w-full appearance-none text-xs px-2.5 py-1.5 pr-7 rounded-lg border border-theme bg-theme-surface text-theme-primary focus:outline-none focus:ring-2 focus:ring-teamly-dark/50 dark:focus:ring-teamly/50"
            >
        ...
        <button
          type="button"
          onClick={onBack}
          className="flex items-center gap-1.5 px-3 py-2.5 text-xs font-medium text-teamly-dark dark:text-teamly hover:bg-teamly-dark/10 dark:hover:bg-teamly/10 rounded-lg border border-theme transition-colors whitespace-nowrap"
        >
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => toggleAll(true)} className="text-xs text-teamly-dark dark:text-teamly hover:underline">
            Select all
          </button>
      ...
      {/* Unmatched sites warning (hidden when bulk site override is active) */}
      {!bulkSiteId && (() => {
          ...
          <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
            ...
            <div>
              ...
              <p className="text-xs text-theme-tertiary mt-1">
                These team members will be imported without a home site. You can assign sites later.
              </p>
            </div>
          </div>
        );
      ...
      <div className="flex items-center justify-between pt-4 border-t border-theme">
        ...
        <Button
          ...
          className="bg-teamly-dark dark:bg-teamly text-white dark:text-[#1C1916] hover:opacity-90 disabled:opacity-40"
        >
          Import {counts.included} Team Member{counts.included !== 1 ? 's' : ''}
        </Button>
      </div>
    </div>
  );
