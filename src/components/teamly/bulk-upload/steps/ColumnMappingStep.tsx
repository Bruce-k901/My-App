...
export function ColumnMappingStep({
  ...
  const requiredFields = TEAMLY_IMPORT_FIELDS.filter((f) => f.required);
  const missingRequired = requiredFields.filter(
    (f) => !usedTargets.has(f.key)
  );

  // Check for name mapping: full_name directly, OR first+last name virtual fields
  const hasFullName = usedTargets.has('full_name');
  const hasFirstName = usedTargets.has('_first_name');
  const nameIsMapped = hasFullName || hasFirstName;

  const canProceed =
    usedTargets.has('email') && nameIsMapped;
  ...
  const handleAutoDetect = () => {
    // Re-run auto-detection by importing the function
    import('@/lib/bulk-import/parser').then(({ autoMapColumns }) => {
      const headers = mappings.map((m) => m.sourceColumn);
      const sampleRows = mappings.map((m) => {
        const obj: Record<string, string> = {};
        obj[m.sourceColumn] = m.sampleValues[0] || '';
        return obj;
      });
      // Build a single combined sample row
      ...
      const newMappings = autoMapColumns(headers, TEAMLY_IMPORT_FIELDS, combinedSamples);
      // Preserve sample values from original mappings
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-xs text-teamly-dark dark:text-teamly hover:underline"
          >
            Auto-detect
          </button>
          <button
      ...
      {!canProceed && (
        <div className="rounded-lg bg-amber-500/10 border border-amber-500/20 p-3">
          <p className="text-xs text-amber-600 dark:text-amber-400">
            ...
            {!nameIsMapped && <strong>Full Name</strong>}
            {!nameIsMapped && !usedTargets.has('email') && ', '}
            {!usedTargets.has('email') && <strong>Email</strong>}
          </p>
        </div>
      )}
      ...
      <div className="max-h-[50vh] overflow-y-auto space-y-2">
        {mappings.map((mapping, index) => (
          ...
          >
            ...
            {/* Arrow */}
            <div className="pt-1.5 text-theme-tertiary">→</div>
            ...
            <div className="w-48 flex-shrink-0">
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none text-sm px-3 py-1.5 pr-8 rounded-lg border border-theme bg-theme-surface text-theme-primary focus:outline-none focus:ring-2 focus:ring-teamly-dark/50 dark:focus:ring-teamly/50"
                >
                  <option value="">Skip this column</option>
                  <option
                    value="_first_name"
                    disabled={usedTargets.has('_first_name') && mapping.targetField !== '_first_name'}
                  >
                    First Name (→ Full Name)
                  </option>
                  <option
                    value="_last_name"
                    disabled={usedTargets.has('_last_name') && mapping.targetField !== '_last_name'}
                  >
                    Last Name (→ Full Name)
                  </option>
                  {TEAMLY_IMPORT_FIELDS.map((field) => (
                    <option
                      key={field.key}
                      value={field.key}
                      disabled={
