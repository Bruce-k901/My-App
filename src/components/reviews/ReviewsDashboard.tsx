  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-theme-primary">Reviews & Appraisals</h1>
          <p className="text-theme-secondary mt-1">Manage employee reviews and performance appraisals</p>
        </div>
      ...
      {!isManagerOrAdmin && schedulesWithoutTemplates.length > 0 && (
        <div className="bg-module-fg/[0.06] border border-module-fg/15 rounded-lg p-4">
          <div className="flex items-center gap-3">
      ...
      <div className="bg-theme-surface border border-theme rounded-lg">
        <div className="p-4 border-b border-theme flex items-center justify-between">
          <h2 className="font-semibold text-theme-primary">My Upcoming Reviews</h2>
          <Link href="/dashboard/people/reviews/my-reviews" className="text-sm text-module-fg hover:underline">
            View all →
          </Link>
        </div>
        <div className="divide-y divide-theme">
          {myUpcoming.length === 0 ? (
            <div className="p-8 text-center">
              <Calendar className="w-10 h-10 text-theme-tertiary mx-auto mb-2" />
              <p className="text-theme-secondary">No upcoming reviews scheduled</p>
            </div>
          ) : (
...
function StatsCard({ title, value, icon: Icon, iconColor, bgColor, href }: { 
  ...
  return (
    <Link href={href}>
      <div className="bg-theme-surface border border-theme rounded-lg p-4 hover:border-theme-hover transition-colors cursor-pointer shadow-sm dark:shadow-none">
...
function ReviewScheduleCard({ schedule }: { schedule: EmployeeReviewSchedule }) {
  ...
  return (
    <Link
      ...
      className={`block p-4 hover:bg-theme-hover transition-colors ${isOverdue ? 'border-l-4 border-l-red-500 dark:border-l-red-400' : !hasTemplate ? 'border-l-4 border-l-amber-500 dark:border-l-amber-400' : ''}`}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-module-fg flex items-center justify-center text-white text-sm font-medium flex-shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-theme-primary font-medium">{schedule.employee?.full_name || 'Employee'}</h3>
              <span className="text-theme-tertiary">-</span>
              <span className="text-theme-secondary">{schedule.template?.name || schedule.title || 'No Template'}</span>
            </div>
            <div className="flex items-center gap-3 mt-1 text-sm text-theme-secondary">
              ...
              {schedule.due_date && (
                <span className={isOverdue ? 'text-red-600 dark:text-red-400' : 'text-theme-secondary'}>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="px-2 py-0.5 bg-module-fg/10 text-module-fg rounded text-xs border border-module-fg/20">
