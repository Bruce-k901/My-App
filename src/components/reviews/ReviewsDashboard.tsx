  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">Reviews & Appraisals</h1>
          <p className="text-neutral-400 mt-1">Manage employee reviews and performance appraisals</p>
        </div>
      ...
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-lg">
        <div className="p-4 border-b border-white/[0.06] flex items-center justify-between">
          <h2 className="font-semibold text-white">My Upcoming Reviews</h2>
          <Link href="/dashboard/people/reviews/my-reviews" className="text-sm text-[#EC4899] hover:underline">
            View all →
          </Link>
        </div>
        <div className="divide-y divide-white/[0.06]">
          {myUpcoming.length === 0 ? (
            <div className="p-8 text-center">
              <Calendar className="w-10 h-10 text-neutral-500 mx-auto mb-2" />
              <p className="text-neutral-400">No upcoming reviews scheduled</p>
            </div>
          ) : (
...
function StatsCard({ title, value, icon: Icon, iconColor, bgColor, href }: { 
  ...
  return (
    <Link href={href}>
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-lg p-4 hover:border-white/[0.1] transition-colors cursor-pointer">
...
function ReviewScheduleCard({ schedule }: { schedule: EmployeeReviewSchedule }) {
  ...
  return (
    <Link
      ...
      className={`block p-4 hover:bg-white/[0.02] transition-colors ${isOverdue ? 'border-l-4 border-l-red-500' : !hasTemplate ? 'border-l-4 border-l-amber-500' : ''}`}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#EC4899] to-blue-500 flex items-center justify-center text-white text-sm font-medium flex-shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-white font-medium">{schedule.employee?.full_name || 'Employee'}</h3>
              <span className="text-neutral-500">-</span>
              <span className="text-neutral-300">{schedule.template?.name || schedule.title || 'No Template'}</span>
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {isOverdue && (
            <span className="px-2 py-0.5 bg-red-500/20 text-red-400 rounded text-xs border border-red-500/30">
              Overdue
            </span>
          )}
