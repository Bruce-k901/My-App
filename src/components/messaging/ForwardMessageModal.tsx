...
export default function ForwardMessageModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 dark:bg-black/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-[#141823] border border-theme rounded-lg w-full max-w-md max-h-[80vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-theme">
          <h2 className="text-lg font-semibold text-theme-primary">Forward Message</h2>
          <button
            onClick={onClose}
            className="p-1 rounded hover:bg-theme-muted text-theme-secondary hover:text-theme-primary transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
            ...
            availableConversations.map((conv) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {isSelected && <Check className="w-3 h-3 text-theme-primary" />}
                  </div>
                  <div className="flex-1 text-left min-w-0">
                    <div className="text-sm font-medium text-theme-primary truncate">{name}</div>
                    {conv.last_message && (
        ...
        <div className="p-4 border-t border-theme flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              onClick={onClose}
              className="px-4 py-2 bg-transparent text-theme-secondary hover:text-theme-primary border border-gray-300 dark:border-white/[0.06] rounded-lg transition-colors"
