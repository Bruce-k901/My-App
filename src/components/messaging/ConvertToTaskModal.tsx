...
export default function ConvertToTaskModal({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/40 dark:bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#1a1d2e] border border-theme rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-theme">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-theme-primary">Convert to Task</h2>
              <p className="text-sm text-theme-secondary">Create a trackable task from this message</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            {sites.length === 0 && (
              <p className="text-xs text-theme-tertiary mt-1">No sites available. Please create a site first.</p>
            )}
          </div>
        ...
        <div className="flex items-center justify-end gap-3 p-6 border-t border-theme">
          <button
            onClick={onClose}
            className="px-6 py-2.5 bg-gray-100 dark:bg-white/[0.03] border border-gray-300 dark:border-white/[0.06] rounded-lg text-theme-secondary hover:bg-gray-200 dark:hover:bg-white/[0.06] transition-colors"
            disabled={loading}
          >
            Cancel
          </button>
