...
export default function MonitorDurationModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-theme-surface border border-theme rounded-xl max-w-md w-full shadow-lg">
        {/* Header */}
        <div className="border-b border-theme p-4 sm:p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-orange-100 dark:bg-orange-500/10 rounded-lg">
              <Clock className="h-5 w-5 text-orange-600 dark:text-orange-400" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-theme-primary">Schedule Monitoring</h2>
 <p className="text-sm text-gray-600 dark:text-theme-tertiary mt-1">
          ...
          <button
            onClick={onClose}
 className="text-gray-600 dark:text-theme-tertiary hover:text-gray-900 transition-colors p-2 hover:bg-theme-muted rounded-lg"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
                ...
                <button
                  ...
                  className={`p-4 rounded-lg border-2 transition-all text-left ${
                    selectedDuration === duration
                      ...
                      : 'border-theme bg-gray-50 dark:bg-white/[0.03] hover:border-orange-300 dark:hover:border-orange-500/50'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-theme-primary font-medium">{formatDuration(duration)}</p>
                    </div>
                    {selectedDuration === duration && (
        ...
        {/* Footer */}
        <div className="border-t border-theme p-4 sm:p-6 flex gap-3">
          <Button
          ...
          <Button
            onClick={onClose}
            className="px-6 py-3 bg-gray-50 dark:bg-white/[0.03] border border-gray-300 dark:border-white/[0.06] text-theme-primary/90 rounded-lg hover:bg-gray-100 dark:hover:bg-white/[0.06] transition-all"
          >
            Cancel
          </Button>
        </div>
