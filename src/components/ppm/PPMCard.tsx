...
export default function PPMCard({ asset, onClick, highlighted = false }: PPMCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-white dark:bg-white/[0.02] backdrop-blur-md rounded-xl border-2 p-6 hover:bg-theme-surface-elevated dark:hover:bg-white/[0.04] transition-all duration-200 cursor-pointer group shadow-sm dark:shadow-none ${
        highlighted
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-semibold text-theme-primary truncate group-hover:text-module-fg dark:group-hover:text-module-fg transition-colors">
            {cleanAsset.name}
          </h3>
          <p className="text-sm text-theme-tertiary mt-1">{cleanAsset.category_name || 'Unknown Category'}</p>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-theme-secondary truncate">
            {cleanAsset.site_name || (
              <span className="text-theme-tertiary italic">No site linked</span>
            )}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-theme-secondary truncate">
            {cleanAsset.contractor_name || (
              <span className="text-theme-tertiary italic">Unassigned</span>
            )}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-theme-secondary">{frequencyText}</span>
        </div>
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-theme">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-theme-tertiary mb-1">Last Service</p>
          ...
          <div>
            <p className="text-theme-tertiary mb-1">Next Service</p>
            <div className="flex items-center gap-1">
              <Calendar className="h-3 w-3 text-theme-tertiary" />
