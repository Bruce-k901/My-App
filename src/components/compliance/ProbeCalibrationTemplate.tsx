...
interface ProbeCalibrationTemplateProps {
...
1. Locate each temperature probe listed below
2. Prepare two calibration solutions:
   - Boiling water (100°C)
   - Ice water (0°C)
3. For each probe, perform calibration checks:
   a. Insert probe into boiling water
   b. Wait 30 seconds for reading to stabilize
   c. Record the reading (should be 100°C ±1°C)
   d. Insert probe into ice water
   e. Wait 30 seconds for reading to stabilize
   f. Record the reading (should be 0°C ±1°C)
4. Record Pass if both readings are within ±1°C tolerance, Fail if outside tolerance
5. If any probe fails calibration:
   - Note the issue in the notes field
   - Remove from service until recalibrated
   - Escalate using the Monitor or Callout option
...
${validEquipment.map(eq => {
  ...
  const loadAssets = async () => {
    ...
    const probeAssets = (data || []).filter(asset => {
      if (!asset.category) return false;
      const category = asset.category.toLowerCase();
      return category.includes('probe') || category.includes('thermometer') || category.includes('temperature') || category.includes('calibration');
    });
    setAssets((probeAssets.length > 0 ? probeAssets : (data || [])) as unknown as Asset[]);
  };
  ...
  const loadDraftData = async (templateId: string) => {
    try {
      ...
      const equipmentField = fields?.find(f => f.field_name === "probe_name");
    ...
    return [
      {
        ...
        field_name: "probe_name",
        field_label: "Temperature Probe",
        field_type: "select",
        is_required: true,
        display_order: 1,
        help_text: "Select the temperature probe being calibrated",
      ...
      {
        task_template_id: templateId,
        field_name: "boiling_point_reading",
        field_label: "Boiling Point Reading (°C)",
        field_type: "number",
        is_required: true,
        min_value: 95,
        max_value: 105,
        display_order: 2,
        help_text: "Reading in boiling water (100°C ±1°C tolerance)"
      },
      {
        task_template_id: templateId,
        field_name: "ice_point_reading",
        field_label: "Ice Point Reading (°C)",
        field_type: "number",
        is_required: true,
        min_value: -2,
        max_value: 2,
        display_order: 3,
        help_text: "Reading in ice water (0°C ±1°C tolerance)"
      },
      {
        task_template_id: templateId,
        field_name: "calibration_result",
        field_label: "Calibration Result",
        field_type: "pass_fail",
        is_required: true,
        display_order: 4,
        help_text: "Pass if both readings are within ±1°C tolerance"
      },
      {
        task_template_id: templateId,
        field_name: "notes",
  ...
  const handleSave = async () => {
    const validEquipment = equipmentRows.filter(row => row.assetId !== "");
    if (validEquipment.length === 0) {
      alert("Please select at least one temperature probe");
      return;
    }
    if (!profile?.company_id) {
    ...
    try {
      const templateName = "Calibrate temperature probes (Draft)";
      const templateSlug = `probe-calibration-draft-${Date.now()}`;
...
${validEquipment.map(eq => {
      ...
      const templateData = {
        ...
        name: templateName,
        slug: templateSlug,
        description: "Monthly calibration verification of temperature probes using boiling and ice water method - Draft",
  ...
  const handleSaveAndDeploy = async () => {
    ...
    try {
      const templateName = "Calibrate temperature probes";
      const templateSlug = editingTemplateId ? undefined : `probe-calibration-${Date.now()}`;
      const defaultInstructions = `How to successfully carry out this task:
          ...
          <div className="flex items-start gap-3 flex-1">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="text-lg font-semibold">Calibrate temperature probes</h3>
              </div>
              <p className="text-theme-tertiary text-sm mb-3">
                Monthly calibration verification of temperature probes using boiling and ice water method. Tolerance: ±1°C.
              </p>
              
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                <div>
                  <span className="text-theme-tertiary">Regulation:</span>
                  <p className="text-theme-primary font-medium">Food Safety Act</p>
                </div>
                <div>
                  <span className="text-theme-tertiary">Frequency:</span>
                  <p className="text-theme-primary font-medium">Monthly</p>
                </div>
                <div>
                ...
                <div>
                  <span className="text-theme-tertiary">Tolerance:</span>
                  <p className="text-theme-primary font-medium">±1°C</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <label className="block text-sm font-medium">
                  Temperature Probes to Calibrate
                </label>
                <button
                  type="button"
                ...
                {equipmentRows.map((row) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {assets.length === 0 ? (
                        <option value="" disabled>No temperature probes found</option>
                      ) : (
                        assets.map(asset => (
