...
import { Wind, Edit2, X } from '@/components/ui/icons';
...
interface ExtractionServiceTemplateProps {
  editTemplateId?: string;
  onSave?: () => void;
}

export function ExtractionServiceTemplate({ editTemplateId, onSave }: ExtractionServiceTemplateProps = {}) {
...
1. Locate each extraction and ventilation system listed below
2. Visually inspect the system for:
   - Cleanliness of filters and ducting
   - Condition of fans and motors
   - Proper operation of switches and controls
3. Test system operation:
   - Turn on extraction/ventilation system
   - Verify adequate airflow
   - Check for unusual noise or vibration
   - Verify grease filters are clean and properly fitted
4. Record the service result (Pass/Fail)
5. Record service certificate number if applicable
6. If any system fails or requires attention:
   - Note the issue in the notes field
   - Escalate immediately using the Callout option
   - Do not use the system until serviced
...
${validEquipment.map(eq => {
  ...
  const loadAssets = async () => {
    ...
    const extractionAssets = (data || []).filter(asset => {
      if (!asset.category) return false;
      const category = asset.category.toLowerCase();
      return category.includes('extraction') || category.includes('ventilation') || category.includes('vent') || category.includes('fan');
    });
    setAssets((extractionAssets.length > 0 ? extractionAssets : (data || [])) as unknown as Asset[]);
  };
  ...
  const loadDraftData = async (templateId: string) => {
    try {
      ...
      const equipmentField = fields?.find(f => f.field_name === "extraction_system");
    ...
    return [
      {
        ...
        field_name: "extraction_system",
        field_label: "Extraction / Ventilation System",
        field_type: "select",
        is_required: true,
        display_order: 1,
        help_text: "Select the extraction or ventilation system being serviced",
      ...
      {
        task_template_id: templateId,
        field_name: "service_result",
        field_label: "Service Result",
        field_type: "pass_fail",
        is_required: true,
        display_order: 2,
        help_text: "Pass if system is operational and clean, Fail if service required"
      },
      {
        task_template_id: templateId,
        field_name: "certificate_number",
        field_label: "Service Certificate Number",
        field_type: "text",
        is_required: false,
        display_order: 3,
        help_text: "Service certificate number if applicable"
      },
      {
        task_template_id: templateId,
        field_name: "notes",
  ...
  const handleSave = async () => {
    const validEquipment = equipmentRows.filter(row => row.assetId !== "");
    if (validEquipment.length === 0) {
      alert("Please select at least one extraction/ventilation system");
      return;
    }
    if (!profile?.company_id) {
    ...
    try {
      const templateName = "Service extraction and ventilation systems (Draft)";
      const templateSlug = `extraction-service-draft-${Date.now()}`;
...
${validEquipment.map(eq => {
      ...
      const templateData = {
        ...
        name: templateName,
        slug: templateSlug,
        description: "Biannual service and verification of extraction and ventilation systems - Draft",
        category: "h_and_s",
        frequency: "quarterly",
        time_of_day: "anytime",
  ...
  const handleSaveAndDeploy = async () => {
    ...
    try {
      const templateName = "Service extraction and ventilation systems";
      const templateSlug = editingTemplateId ? undefined : `extraction-service-${Date.now()}`;
      const defaultInstructions = `How to successfully carry out this task:
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-3 flex-1">
            <div className="p-2 rounded-lg bg-magenta-500/10 border border-magenta-500/20">
              <Wind className="w-5 h-5 text-magenta-400" />
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="text-lg font-semibold">Service extraction and ventilation systems</h3>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <label className="block text-sm font-medium">
                  Extraction / Ventilation Systems to Service
                </label>
                <button
                  type="button"
                ...
                {equipmentRows.map((row) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {assets.length === 0 ? (
                        <option value="" disabled>No extraction systems found</option>
                      ) : (
                        assets.map(asset => (
