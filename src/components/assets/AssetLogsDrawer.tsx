...
export default function AssetLogsDrawer({ asset, open, onClose }: AssetLogsDrawerProps) {
  ...
  const getLogTypeColor = (type: string) => {
    switch (type) {
      ...
      default:
        return 'bg-gray-100 dark:bg-neutral-900/30 text-gray-600 dark:text-neutral-400 border-neutral-400';
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      {/* Drawer */}
      <div className="w-96 bg-white dark:bg-neutral-900 border-l border-gray-200 dark:border-neutral-700 flex flex-col shadow-xl dark:shadow-none">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-neutral-700">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Asset Logs</h2>
            <p className="text-sm text-gray-500 dark:text-neutral-400">{asset.name}</p>
          </div>
          <Button
            variant="outline"
            size="sm"
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            ...
            <div className="flex items-center justify-center py-8">
              <p className="text-gray-500 dark:text-neutral-400">No logs found for this asset.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {logs.map((log) => (
                <div
                  key={log.id}
                  className="p-4 rounded-lg border border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800/50"
                >
