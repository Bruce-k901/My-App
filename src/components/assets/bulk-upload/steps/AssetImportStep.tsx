...
export function AssetImportStep({
  isImporting,
  progress,
  result,
  error,
  onBack,
}: AssetImportStepProps) {
  ...
  if (isImporting) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-4">
        ...
        <p className="text-sm text-theme-primary font-medium">Importing assets...</p>
        <div className="w-64 h-2 bg-theme-muted rounded-full overflow-hidden">
          <div
            className="h-full bg-assetly-dark dark:bg-assetly rounded-full transition-all duration-300"
            style={{ width: `${pct}%` }}
          />
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-center py-6 space-y-3">
        ...
        <p className="text-sm text-theme-secondary text-center max-w-md">
          {result.created > 0
            ? `${result.created} asset${result.created !== 1 ? 's have' : ' has'} been added.`
            : 'No new assets were imported.'}
      ...
      <div className="flex items-center justify-center gap-3 pt-2">
        ...
        <Button
          ...
          className="bg-assetly-dark dark:bg-assetly text-white dark:text-[#1C1916] hover:opacity-90"
        >
          View Assets
        </Button>
      </div>
