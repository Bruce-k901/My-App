    ...
    <div ref={searchRef} className="relative w-full max-w-2xl">
          ...
          {results.map((result, index) => (
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  {result.registered_address && (
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <p className="text-sm text-slate-600 truncate">{result.registered_address}</p>
                    </div>
                  )}
                ...
                <Plus className="h-4 w-4 text-slate-400 flex-shrink-0 ml-2" />
              </div>
            </div>
          ))}
        </div>
      ...
      {showResults && results.length === 0 && !loading && query.trim() && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg p-4">
          <p className="text-slate-500 text-center">No contractors found for "{query}"</p>
        </div>
      )}
    </div>
  );
